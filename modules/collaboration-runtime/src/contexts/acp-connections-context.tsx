...
type Action =
  ...
  | {
      ...
      contextKey: string
      groupName: string
      members: GroupCollaborationMemberInfo[]
...
function groupCollaborationEntries(
  groupName: string,
  members: GroupCollaborationMemberInfo[],
  invalidMentions: string[]
): PlanEntryInfo[] {
  const entries: PlanEntryInfo[] = members.map((member) => ({
    content: `Delegate in ${groupName}: ${AGENT_LABELS[member.agent_type]} - ${member.role}`,
    priority: "medium",
    status: "in_progress",
  }))
  ...
  for (const mention of invalidMentions) {
    entries.push({
      content: `Skipped @${mention}: not an active member of ${groupName}`,
      priority: "low",
      status: "pending",
    })
  }
  ...
  if (entries.length === 0) {
    entries.push({
      content: `No matching group members found for ${groupName}`,
      priority: "low",
      status: "pending",
    })
  }
...
function connectionsReducer(
  ...
  switch (action.type) {
    ...
    case "GROUP_COLLABORATION_PLAN": {
      ...
      const entries = groupCollaborationEntries(
        action.groupName,
        action.members,
        action.invalidMentions
      )
      const next = new Map(state)
      next.set(action.contextKey, {
        ...conn,
        liveMessage: {
          ...prev,
          content: [...nonPlanContent, { type: "plan", entries }],
        },
        claudeApiRetry: null,
      })
      return next
...
export function AcpConnectionsProvider({ children }: { children: ReactNode }) {
  ...
  const claudeRetryAlertKeysRef = useRef(new Set<string>())
    ...
    (contextKey: string, e: EventEnvelope) => {
      switch (e.type) {
        ...
        case "claude_sdk_message":
          flushStreamingQueue()
          {
            const retry = parseClaudeApiRetryEvent(e)
            dispatch({
              type: "CLAUDE_API_RETRY",
              contextKey,
              retry,
            ...
            if (retry) {
              const alertKey = `${contextKey}:${retry.sessionId}`
              if (!claudeRetryAlertKeysRef.current.has(alertKey)) {
                claudeRetryAlertKeysRef.current.add(alertKey)
                const nc = storeRef.current.connections.get(contextKey)
                const agentType = nc?.agentType ?? "claude_code"
                const agentLabel = AGENT_LABELS[agentType]
                const rawError = retry.error?.trim()
                const normalizedError = rawError?.toLowerCase()
                const errorMessage =
                  !rawError || normalizedError === "unknown"
                    ? t("claudeApiRetry.fallbackError")
                    : rawError
                const statusMessage =
                  retry.errorStatus !== null && retry.errorStatus !== undefined
                    ? ` HTTP ${Math.trunc(retry.errorStatus)}`
                    : ""
        ...
        case "permission_request":
          ...
          // Send OS notification when permission approval is needed
          {
            ...
            if (nc) {
              ...
              const title = fn ? `${fn} - Archipelago` : "Archipelago"
        ...
        case "group_collaboration_plan":
          ...
          dispatch({
            ...
            members: e.members,
            invalidMentions: e.invalid_mentions,
          })
          break
        case "turn_complete": {
          flushStreamingQueue()
  ...
  // (ARCHIPELAGO_ACP_IDLE_TIMEOUT_SECS, default 180s) would reap connections
