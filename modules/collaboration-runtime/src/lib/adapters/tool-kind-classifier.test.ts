...
describe("isAgentLikeToolName", () => {
  ...
  it("matches delegate_to_agent across host naming conventions", () => {
    ...
    // Claude Code style
    expect(isAgentLikeToolName("mcp__archipelago-delegate__delegate_to_agent")).toBe(
      true
    )
    expect(isAgentLikeToolName("mcp__archipelago-delegate__delegate_to_agent")).toBe(true)
    // Codex live ACP style (server/tool)
    expect(isAgentLikeToolName("archipelago-delegate/delegate_to_agent")).toBe(true)
