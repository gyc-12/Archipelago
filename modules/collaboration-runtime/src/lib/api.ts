...
import { getArchipelagoToken, redirectToArchipelagoLogin } from "./transport/web-auth"
...
export async function acpPrompt(
  ...
  collaborationMode: GroupCollaborationMode = "mention"
): Promise<void> {
...
export async function listGroups(): Promise<GroupChatWithAgents[]> {
  return getTransport().call("list_groups")
}
...
export const ISLAND_GROUP_UPSERTED_EVENT = "island://group-upserted"
export const ISLAND_GROUP_DELETED_EVENT = "island://group-deleted"
export const ISLAND_AGENT_UPSERTED_EVENT = "island://agent-upserted"
export const ISLAND_AGENT_DELETED_EVENT = "island://agent-deleted"
...
/**
 * Detect, per Archipelago-supported agent, whether the HyperFrames skills are already
...
export async function uploadAttachment(
  ...
  if (file.size === 0) {
    ...
    // (the server records it under `~/.archipelago/uploads/<bucket>/...`), and
  ...
  const token = getArchipelagoToken()
...
// archipelago-server bound to the current window. The Tauri-native drag-drop event
...
export interface DelegationSettings {
  enabled: boolean
  depth_limit: number
  /** Optional per-agent overrides applied when archipelago-mcp spawns a subagent.
 ...
 * archipelago-mcp will receive when a subagent is spawned for delegation.
 *
