...
function AlertLevelIcon({ level }: { level: AlertLevel }) {
  if (level === "error") {
    return <AppleIcon name="warning" className="size-3 shrink-0 text-red-500" />
  }
  return <AppleIcon name="warning" className="size-3 shrink-0 text-yellow-500" />
}

function shouldAutoOpenAlert(alert: Alert | undefined): boolean {
  if (!alert) return false
  if (alert.level === "error") return true
  return (
    alert.actions?.some((action) => action.kind === "open_agents_settings") ??
    false
  )
}
...
export function StatusBarAlerts() {
  ...
  const [closedAutoOpenAlertIds, setClosedAutoOpenAlertIds] = useState<
    Set<string>
  >(() => new Set())
  ...
  const autoOpenAlertId = shouldAutoOpenAlert(latestAlert)
    ? latestAlert?.id
    : null
  const autoOpen =
    !!autoOpenAlertId && !closedAutoOpenAlertIds.has(autoOpenAlertId)
  const open = hasAlerts && (manualOpen || autoOpen)

  const handleOpenChange = (nextOpen: boolean) => {
    if (!nextOpen && autoOpenAlertId) {
      setClosedAutoOpenAlertIds((previous) => {
        const next = new Set(previous)
        next.add(autoOpenAlertId)
        return next
      })
    }
      ...
      <PopoverContent side="top" align="end" className="w-80 p-3">
        <div className="flex items-center justify-between mb-2">
          ...
          {hasAlerts && (
            ...
            >
              <AppleIcon name="trash" className="size-3" />
            </button>
          )}
        </div>
        {!hasAlerts ? (
