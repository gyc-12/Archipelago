...
const agentIconFrameClass =
  "inline-flex shrink-0 items-center justify-center rounded-full border border-border/70 bg-[var(--archipelago-icon-surface)] text-foreground"

export function AgentIcon({ agentType, className }: AgentIconProps) {
  const ColorIcon = COLOR_ICONS[agentType]
  if (ColorIcon) {
    return (
      <span className={cn(agentIconFrameClass, className)}>
        <ColorIcon size="78%" />
      </span>
    )
  }
  ...
  return (
    <span className={cn(agentIconFrameClass, className)}>
      <span
        className={cn("h-1/2 w-1/2 rounded-full", AGENT_COLORS[agentType])}
      />
    </span>
  )
}
