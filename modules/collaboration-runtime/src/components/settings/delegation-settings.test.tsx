...
function settings(
  ...
  // Mirror the backend default (DelegationSettings::default) so tests that
  // don't care about the toggle reflect the production wire shape.
  return {
    enabled: true,
    depth_limit: 1,
    agent_defaults: {},
...
describe("DelegationSettingsSection", () => {
  ...
  it("reflects backend default (enabled): switch on, depth input enabled", async () => {
    mockGetDelegationSettings.mockResolvedValue(settings())
    ...
    expect(enableSwitch).toHaveAttribute("data-state", "checked")
    expect(depthInput).not.toBeDisabled()
  })

  it("disables depth input when backend explicitly disables delegation", async () => {
