...
function AddressCard({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="group relative flex items-center rounded-[14px] border bg-muted/40 px-3 py-2">
        ...
        <div className="ml-2 flex shrink-0 items-center gap-1">
          <button
            ...
            className="inline-flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground hover:bg-accent hover:text-accent-foreground"
            title={t("open")}
          >
            <AppleIcon name="external" className="h-3.5 w-3.5" />
          </button>
        </div>
      </div>
    </div>
        ...
        <div className="ml-2 flex shrink-0 items-center gap-1">
          {!disabled && (
            <button
              ...
              title={t("regenerate")}
            >
              <AppleIcon name="refresh" className="h-3.5 w-3.5" />
            </button>
          )}
          <button
            type="button"
          ...
          <button
            ...
            onClick={handleCopy}
            disabled={!value}
            className="inline-flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground hover:bg-accent hover:text-accent-foreground disabled:cursor-not-allowed disabled:opacity-40"
            title={t("copy")}
          >
            {copied ? (
              <AppleIcon name="checkCircle" className="h-3.5 w-3.5 text-green-500" />
            ) : (
              <AppleIcon name="copy" className="h-3.5 w-3.5" />
            )}
          </button>
        </div>
      </div>
    ...
    <ScrollArea className="h-full">
      <div className="space-y-5 p-3 md:p-4">
        <div className="flex items-start gap-3">
          <AppleIconTile name="webService" tone="green" className="mt-0.5" />
          <div>
        ...
        <section className="space-y-4 rounded-xl border bg-card p-4">
          {showStaleBanner && (
            <div className="flex items-start gap-3 rounded-[14px] border border-amber-500/40 bg-amber-500/10 p-3">
              <AppleIcon name="warning" className="mt-0.5 h-4 w-4 shrink-0 text-amber-600 dark:text-amber-400" />
          ...
          <div className="flex items-center gap-4">
            ...
            <input
              ...
              min={1024}
              max={65535}
              className="flex h-9 w-32 rounded-full border border-border/70 bg-card px-3 py-1 text-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20 disabled:opacity-50"
            />
          </div>
