...
import { extractArtifactsFromText } from "@/lib/artifact-detection"
...
function ToolGlyph({ name }: { name: AppleIconName }) {
  return <AppleIcon name={name} className={ICON_CLASS} />
}

function getTaskToolIcon(input: string | null): ReactNode {
  if (!input) return <ToolGlyph name="todo" />
  const t = extractJsonField(input, "subagent_type")?.toLowerCase()
  if (!t) return <ToolGlyph name="todo" />
  if (t.includes("explore")) return <ToolGlyph name="search" />
  if (t.includes("plan")) return <ToolGlyph name="todo" />
  if (t.includes("bash")) return <ToolGlyph name="terminal" />
  return <ToolGlyph name="wrench" />
}
...
function getToolIcon(
  ...
  if (name === "read" || name === "read file") return <ToolGlyph name="file" />
  if (name === "edit") return <ToolGlyph name="edit" />
  if (name === "write" || name === "notebookedit")
    return <ToolGlyph name="plus" />
  if (name === "bash" || name === "exec_command")
    return <ToolGlyph name="terminal" />
  if (name === "apply_patch") return <ToolGlyph name="edit" />
  if (name === "glob" || name === "grep") return <ToolGlyph name="search" />
  if (name === "memory_recall") return <ToolGlyph name="thinking" />
  if (name === "webfetch" || name === "websearch")
    return <ToolGlyph name="globe" />
  if (name === "todowrite") return <ToolGlyph name="todo" />
  if (name === "task") return getTaskToolIcon(input ?? null)
  if (name === "taskcreate" || name === "taskupdate" || name === "tasklist")
    return <ToolGlyph name="todo" />
  if (name === "agent") return getTaskToolIcon(input ?? null)
  if (name === "skill") return <ToolGlyph name="sparkle" />
  if (
    name === "enterplanmode" ||
    name === "exitplanmode" ||
    name === "switch_mode"
    ...
    return <ToolGlyph name="todo" />
  if (name === "attempt_completion") return <ToolGlyph name="checkCircle" />
  return undefined
}
...
function TodoWriteToolInput({ input }: { input: Record<string, unknown> }) {
  ...
  const statusIcon = (status: string) => {
    ...
    if (status === "in_progress")
      return (
        <span className="size-3.5 shrink-0 rounded-full border border-blue-500 bg-blue-500/20" />
      )
    return (
...
const TextPart = memo(function TextPart({
  ...
  const artifacts = useMemo(
    () => (preserveNewlines ? [] : extractArtifactsFromText(text)),
