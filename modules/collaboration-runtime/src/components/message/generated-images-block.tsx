...
export const GeneratedImagesBlock = memo(function GeneratedImagesBlock({
  ...
  return (
    <div
      className={cn(
        "@container/genimg rounded-[18px] border border-border/50 bg-background/70 p-3 ring-1 ring-black/5 backdrop-blur-xl dark:bg-white/[0.04] dark:ring-white/10",
        className
      )}
    >
      <div className="flex items-center gap-2 text-sm font-semibold tracking-[-0.224px] text-foreground">
        <AppleIconTile
          name="image"
          tone="purple"
      ...
      <div className="mt-2.5 flex flex-col gap-3 @[28rem]/genimg:flex-row @[28rem]/genimg:items-start">
        ...
        {image ? (
          <div className="group relative inline-block shrink-0 overflow-hidden rounded-[16px] border border-border/50 bg-muted/25 ring-1 ring-black/5 dark:ring-white/10">
            <button
              type="button"
            ...
            <button
              ...
              className="absolute right-1 top-1 rounded-full bg-background/80 p-1 text-foreground/80 opacity-0 transition-opacity hover:bg-primary/8 hover:text-foreground group-hover:opacity-100 focus-visible:opacity-100"
              aria-label={t("downloadImage")}
              title={t("downloadImage")}
            >
              <AppleIcon name="download" className="size-3.5" />
            </button>
          </div>
        ) : isFailed ? (
          <div
            className="flex h-64 w-64 max-w-full shrink-0 items-center justify-center rounded-[16px] border border-dashed border-destructive/35 bg-destructive/5 text-xs text-destructive"
            role="status"
            aria-live="polite"
          >
            <div className="flex flex-col items-center gap-1.5">
              <AppleIcon name="warning" className="size-6 opacity-80" />
              <span>{t("imageGenerationFailed")}</span>
            </div>
          </div>
        ) : (
          <div
            className="flex h-64 w-64 max-w-full shrink-0 animate-pulse items-center justify-center rounded-[16px] border border-dashed border-border/50 bg-muted/30 text-xs text-muted-foreground"
            role="status"
            aria-live="polite"
          >
            <div className="flex flex-col items-center gap-1.5">
              <AppleIcon name="image" className="size-6 opacity-60" />
