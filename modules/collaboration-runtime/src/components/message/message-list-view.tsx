...
const CollapsibleSystemMessage = memo(function CollapsibleSystemMessage({
  ...
  return (
    <div className="rounded-[18px] border border-primary/20 bg-primary/5 text-sm">
      <button
        onClick={() => setExpanded(!expanded)}
        className="flex items-center gap-2 w-full px-3 py-2.5 text-left hover:bg-primary/8 transition-colors"
      >
        <AppleIcon
          name={expanded ? "arrowDown" : "arrowRight"}
          className="size-3.5 shrink-0 text-primary"
        />
        <AppleIcon name="warning" className="size-3.5 shrink-0 text-primary" />
        <span className="font-medium text-primary">{t("systemMessage")}</span>
      </button>
      {expanded && (
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <AppleIcon name="spinner" className="size-4 animate-spin" />
          <span>{t("loading")}</span>
        </div>
      </div>
    )
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
              {onReload && (
                ...
                >
                  {reloading ? (
                    ...
                    <AppleIcon name="refresh" className="me-1.5 size-4" />
                  )}
                  {t("errorActionReload")}
                </Button>
              )}
              {onNewSession && (
                <Button size="sm" variant="outline" onClick={onNewSession}>
                  <AppleIcon name="plus" className="me-1.5 size-4" />
