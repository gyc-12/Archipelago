      ...
      <div className="flex flex-wrap gap-1.5">
        {images.map((image, index) => (
          <div
            ...
            className="group relative overflow-hidden rounded-[12px] border border-border/50 bg-background/70 ring-1 ring-black/5 backdrop-blur-xl dark:bg-white/[0.04] dark:ring-white/10"
          >
            <button
              type="button"
            ...
            <button
              ...
              className="absolute right-0.5 top-0.5 rounded-full bg-background/80 p-0.5 text-foreground/80 opacity-0 transition-opacity hover:bg-primary/8 hover:text-foreground group-hover:opacity-100 focus-visible:opacity-100"
              aria-label={t("downloadImage")}
              title={t("downloadImage")}
            >
              <AppleIcon name="download" className="size-3" />
            </button>
          </div>
        ))}
      </div>
