...
export const TabItem = memo(function TabItem({
  ...
  const whileDrag = useMemo(() => ({ scale: 1.01 }), [])
  ...
  return (
    <Reorder.Item
      ...
      className={cn(
        ...
        !isCoarsePointer && "active:opacity-90 active:z-50",
        isTouchSorting && "z-50 bg-background/95 opacity-90 ring-1 ring-primary/25"
      )}
    >
      <ContextMenu>
        <ContextMenuTrigger asChild disabled={isTouchSorting}>
          <div
            ...
            className={cn(
              "group/tab relative flex h-full shrink-0 items-center gap-1.5 rounded-[11px] px-3 text-xs",
              "cursor-pointer select-none border border-transparent transition-colors",
              "hover:bg-primary/8",
              isActive
                ? "border-primary/20 bg-background/90 text-primary ring-1 ring-primary/10"
