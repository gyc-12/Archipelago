...
import {
  ...
  type ContentBlock,
  type EventEnvelope,
  type GroupCollaborationMemberInfo,
  type GroupChatWithAgents,
  type MessageTurn,
  type PromptDraft,
} from "@/lib/types"
import {
...
function collaborationMembersForConversation(
  groups: GroupChatWithAgents[],
  conversationId: number | null
): GroupCollaborationMemberInfo[] | null {
  if (conversationId == null) return null
  for (const group of groups) {
    const current = group.agents.find(
      (agent) => agent.conversationId === conversationId
    )
    if (!current || group.group.primaryAgentId !== current.id) continue
...
const ConversationTabView = memo(function ConversationTabView({
  ...
  useEffect(() => {
    ...
    void listGroups()
      .then((groups) => {
        if (cancelled) return
        setGroupMembers(
          collaborationMembersForConversation(groups, dbConversationId)
        )
      })
      .catch((error) => {
        if (cancelled) return
        console.debug(
          "[ConversationTabView] list groups for collaboration:",
          error
        )
        setGroupMembers(null)
      })
  ...
  return (
    ...
    >
      {isWelcomeMode ? (
        <div className="relative isolate flex h-full min-h-0 flex-col overflow-x-hidden overflow-y-auto bg-[linear-gradient(180deg,rgb(255_255_255_/_35%),rgb(255_255_255_/_0%)_12rem),var(--background)] dark:bg-[linear-gradient(180deg,rgb(255_255_255_/_4%),rgb(255_255_255_/_0%)_12rem),var(--background)]">
          ...
          <div className="mx-auto flex w-full max-w-2xl shrink-0 flex-col gap-6 px-4 py-4">
            ...
            {autoConnectError || agentConnectError ? (
              <button
                ...
                className="w-full cursor-pointer rounded-full border border-destructive/30 bg-destructive/5 px-3 py-2 text-center text-xs text-destructive transition-colors hover:bg-destructive/10"
              >
                <div
        ...
        <div className="flex h-full min-h-0 flex-col bg-background">
          <div className="border-b border-border/70 bg-background/76 px-4 pt-3 pb-2 shadow-[0_1px_0_rgb(255_255_255_/_45%)_inset] backdrop-blur-xl dark:shadow-[0_1px_0_rgb(255_255_255_/_6%)_inset]">
            <AgentSelector
              defaultAgentType={selectedAgent}
              onSelect={handleAgentSelect}
              onFallback={handleAgentFallback}
            ...
            {autoConnectError || agentConnectError ? (
              <button
                type="button"
                onClick={handleOpenAgentsSettings}
                className="mt-2 w-full cursor-pointer rounded-full border border-destructive/30 bg-destructive/5 px-3 py-2 text-center text-xs text-destructive transition-colors hover:bg-destructive/10"
              >
                <div
                  className="overflow-hidden text-ellipsis whitespace-nowrap text-center"
                  title={autoConnectError ?? agentConnectError ?? ""}
      ...
      >
        {canTile && active && (
          <div
            ...
            className="absolute top-2 left-1/2 z-20 flex h-6 w-6 -translate-x-1/2 items-center justify-center rounded-full bg-background/80 text-sidebar-primary ring-1 ring-sidebar-primary/20 backdrop-blur"
          >
