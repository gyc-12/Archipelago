...
function isUnhelpfulClaudeRetryError(error: string | null): boolean {
...
export function ConversationShell({
  ...
  const retryLineText = useMemo(() => {
    ...
    const retryDelaySeconds =
      retry.retryDelayMs !== null && retry.retryDelayMs !== undefined
        ...
        : null
    const fallbackError = tAcp("claudeApiRetry.fallbackError")
    const errorLabel = isUnhelpfulClaudeRetryError(retry.error)
      ? fallbackError
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-background">
      ...
      {!hideInput && (
        <div className="border-t border-border/70 bg-background/78 px-3 py-3 shadow-[0_-1px_0_rgb(255_255_255_/_45%)_inset] backdrop-blur-xl dark:shadow-[0_-1px_0_rgb(255_255_255_/_6%)_inset]">
      ...
      {error && (
        <div className="px-4 py-2 text-xs text-destructive bg-destructive/8 border-t border-destructive/20">
          {error}
        </div>
      )}
    </div>
