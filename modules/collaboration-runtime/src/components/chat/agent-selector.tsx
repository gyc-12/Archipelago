...
export function AgentSelector({
  ...
  const agents = useMemo<AcpAgentInfo[]>(
    () =>
      rawAgents.filter((a) => a.enabled && isUserFacingAgentType(a.agent_type)),
    [rawAgents]
  )
  ...
  if (agents.length === 0) {
    return (
      <div className="rounded-[18px] border border-dashed bg-card/70 px-4 py-3 text-center text-sm text-muted-foreground backdrop-blur-xl">
        <div>{t("noEnabledAgents")}</div>
        {onOpenAgentsSettings ? (
          <button
            type="button"
            onClick={onOpenAgentsSettings}
            className="mt-2 inline-flex items-center rounded-full border px-3 py-1 text-xs text-foreground transition-colors hover:bg-accent cursor-pointer"
          >
  ...
  return (
    <div
      ref={containerRef}
      className="relative inline-flex items-center self-center rounded-full border border-border/70 bg-card/72 p-1 shadow-[inset_0_1px_0_rgb(255_255_255_/_55%)] backdrop-blur-xl dark:shadow-[inset_0_1px_0_rgb(255_255_255_/_8%)]"
    >
      {/* Sliding droplet indicator */}
      {indicator && (
        <div
          className="absolute bottom-1 top-1 rounded-full bg-primary/10 ring-1 ring-primary/20 transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] dark:bg-primary/16"
          style={{
            left: indicator.left,
        ...
        return (
          <button
            ...
            className={cn(
              ...
              isSelected ? "px-3 py-1.5" : "px-2 py-1.5",
