...
describe("FileMentionMenu", () => {
  ...
  it("applies the primary selection style only to the selectedIndex row", () => {
    render(
      <FileMentionMenu
    ...
    expect(buttons[0].className.split(/\s+/)).not.toContain("bg-primary/8")
    expect(buttons[1].className.split(/\s+/)).toContain("bg-primary/8")
    expect(buttons[2].className.split(/\s+/)).not.toContain("bg-primary/8")
  })
