...
interface MessageInputProps {
  ...
  groupMembers?: GroupCollaborationMemberInfo[] | null
}
...
export function MessageInput({
  ...
  // mutates afterwards. A desktop window bound to a remote archipelago-server
  ...
  // Kept so slash autocomplete can still route expert commands correctly.
  ...
  // also show up in `acp_list_agent_skills`. Strip them out so the `$` list is
  // skills-only.
  ...
  const collaborationMembers = useMemo(
    () =>
      (groupMembers ?? []).filter((member) => member.agent_type !== agentType),
    [agentType, groupMembers]
  )
  ...
  //
  // Hide built-in expert command ids from the slash list. The add menu no
  // longer surfaces expert shortcuts, and Codex still gets `$`-triggered
  // non-expert skills because its native command set is very small.
  ...
  // past the end of the merged list (commands + skills), which would make
  ...
  const handleLocalFileInputChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (files.length === 0) return
      void appendFilesFromInput(files).catch((error) => {
        console.error("[MessageInput] local file input failed:", error)
      })
    },
  ...
  const handleGroupMemberSelect = useCallback(
    (member: GroupCollaborationMemberInfo) => {
      const mention = `@${member.agent_type} `
      const current = textRef.current
      const rawPos = textareaRef.current?.selectionStart ?? current.length
      const pos = Math.max(0, Math.min(rawPos, current.length))
      const before = current.slice(0, pos)
      const after = current.slice(pos)
      const insertion = `${before.length > 0 && !/\s$/.test(before) ? " " : ""}${mention}`
  ...
  const actionButtons = isEditingQueueItem ? (
    <div className="flex items-center gap-1">
      ...
      >
        <AppleIcon name="send" className="size-4" />
      </Button>
    </div>
    ...
    <Button
      ...
      title={t("cancel")}
    >
      <AppleIcon name="stop" className="size-4" />
    </Button>
  ) : onForkSend ? (
  ...
  return (
    ...
    >
      ...
      {slashMenuOpen && slashAutocompleteCount > 0 && (
        <div className="absolute bottom-full left-0 right-0 z-50 mb-2 flex max-h-[min(16rem,40dvh)] flex-col overflow-hidden rounded-[18px] border border-border/70 bg-popover/95 shadow-[0_18px_50px_rgb(0_0_0_/_14%)] backdrop-blur-xl">
            ...
            {filteredSlashSkills.map((skill, i) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  className={cn(
                    "flex w-full items-start gap-2 rounded-[11px] px-3 py-2 text-left text-sm",
      ...
      >
        <div
          className={cn(
            ...
            folderBranchPickerAttached
              ? "rounded-[18px] border border-input bg-card/95 focus-within:border-ring focus-within:ring-[2px] focus-within:ring-inset focus-within:ring-ring/35"
              : "rounded-[18px] border border-input bg-card/95 focus-within:border-ring focus-within:ring-[2px] focus-within:ring-ring/35",
            !folderBranchPickerAttached &&
              showDragActive &&
              "ring-1 ring-primary/40",
            className
        ...
        >
            ...
            extraContent={
              <>
                {imageAttachments.map((attachment) => (
                  <div
                    key={attachment.id}
                    className="relative shrink-0 overflow-hidden rounded-lg border border-border/70 bg-secondary"
                  >
                    <button
                      type="button"
                ...
                {resourceAttachments.map((attachment) => (
                  <div
                    key={attachment.id}
                    className="inline-flex h-6 shrink-0 items-center gap-1 rounded-full border border-border/70 bg-secondary px-2 text-[11px] text-muted-foreground"
                  >
          ...
          {collaborationMembers.length > 0 && (
            <div className="flex shrink-0 flex-wrap items-center gap-1.5 border-b border-border/50 px-3 py-2">
              ...
              {collaborationMembers.map((member) => (
                <button
                  key={`${member.agent_id}-${member.agent_type}`}
                  type="button"
                  onClick={() => handleGroupMemberSelect(member)}
                  className="inline-flex h-6 max-w-40 items-center gap-1.5 rounded-full border border-border/70 bg-secondary px-2 text-[11px] text-muted-foreground transition-colors hover:border-primary/30 hover:bg-primary/8 hover:text-foreground"
                  title={`@${member.agent_type} ${member.role}`}
                >
                  <AgentIcon
              ...
              <button
                ...
                className="inline-flex h-6 items-center rounded-full border border-border/70 bg-secondary px-2 text-[11px] font-medium text-muted-foreground transition-colors hover:border-primary/30 hover:bg-primary/8 hover:text-foreground"
              >
                @all
              </button>
            </div>
          ...
          <Textarea
            ...
            className="min-h-0 flex-1 overflow-y-auto rounded-none border-0 bg-transparent px-3 py-3 text-base tracking-[-0.01em] shadow-none focus-visible:border-0 focus-visible:ring-0 md:text-sm"
            autoFocus={autoFocus}
          />
            ...
            <div className="flex min-w-0 items-end gap-1">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <AppleIcon name="attach" className="size-4" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent
                  side="top"
                ...
                >
                  {showNativePaperclip ? (
                    ...
                    <>
                      ...
                      >
                        <AppleIcon name="upload" className="size-4" />
                        {t("attachLocalUpload")}
                      </DropdownMenuItem>
                      <DropdownMenuItem
                  ...
                  >
                    ...
                    >
                      <AppleIcon name="command" className="size-4" />
                      {t("slashCommands")}
                    </DropdownMenuSubTrigger>
                    <DropdownMenuSubContent
          ...
          {showDragActive && (
            <div className="pointer-events-none absolute inset-1 z-20 flex items-center justify-center rounded-[14px] border border-dashed border-primary/50 bg-background/80 text-xs text-muted-foreground">
              {t("dropFilesToAttach")}
            </div>
          )}
        </div>
