...
function StatusIcon({
  ...
  if (status === "completed") {
    return <AppleIcon name="checkCircle" className="h-3.5 w-3.5 text-emerald-500" />
  }
  ...
  return <AppleIcon name="spinner" className="h-3.5 w-3.5 text-muted-foreground" />
}
...
export const AgentPlanOverlay = memo(function AgentPlanOverlay({
    ...
    return (
      <div className="pointer-events-none absolute right-8 top-4 z-20 flex">
        <Button
          ...
          variant="secondary"
          size="sm"
          className="pointer-events-auto cursor-pointer bg-card/85 shadow-[0_10px_28px_rgb(0_0_0_/_10%)] ring-1 ring-border/60 backdrop-blur-xl hover:bg-card"
          onClick={() =>
  ...
  return (
    ...
    >
      <div className="pointer-events-auto w-72 max-w-full overflow-hidden rounded-[20px] border border-border/70 bg-card/88 shadow-[0_18px_52px_rgb(0_0_0_/_14%)] backdrop-blur-xl transition-colors hover:bg-card/95 supports-[backdrop-filter]:bg-card/78 supports-[backdrop-filter]:hover:bg-card/90">
        ...
        <div className="max-h-96 overflow-y-auto p-3 space-y-2">
          {resolvedEntries.map((entry, index) => (
            <div
              ...
              className="rounded-[14px] border border-border/70 bg-background/45 px-2.5 py-2"
            >
