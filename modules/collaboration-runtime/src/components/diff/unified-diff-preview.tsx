...
function HunkSeparator({ hunk }: { hunk: ParsedDiffHunk }) {
  const label =
    hunk.oldStart != null && hunk.oldCount != null
      ...
      : "···"
  return (
    <div className="flex items-center gap-2 border-y border-border/40 bg-muted/25 px-3 py-0.5 font-mono text-[11px] text-muted-foreground/60">
...
export function UnifiedDiffPreview({
  ...
  if (!diffText.trim()) {
    return (
      <div
        className={cn(
          "flex h-full items-center justify-center rounded-[18px] border border-border/50 bg-background/70 text-xs text-muted-foreground",
          className
        )}
      >
        {t("noDiffData")}
  ...
  if (files.length === 0) {
    return (
      <ScrollArea className={cn("h-full", className)} x="scroll">
        <pre className="rounded-[18px] border border-border/50 bg-background/70 p-3 font-mono text-[11px] leading-5 whitespace-pre-wrap text-muted-foreground">
          {diffText}
        </pre>
      </ScrollArea>
    )
      ...
      <div className="space-y-3">
        {files.map((file) => {
          ...
          return (
            <section
              key={file.key}
              className="flex max-h-[420px] flex-col overflow-hidden rounded-[18px] border border-border/50 bg-[rgb(255_255_255_/_78%)] ring-1 ring-black/5 backdrop-blur-xl dark:bg-white/[0.04] dark:ring-white/10"
            >
              <header className="flex shrink-0 items-center gap-2 border-b border-border/45 bg-background/55 px-3 py-2 text-[11px] dark:bg-white/[0.03]">
                <AppleIconTile
                  name={newFile ? "file" : "diff"}
                  tone={newFile ? "blue" : "green"}
                  className="size-6 rounded-[7px]"
                  iconClassName="size-3.5"
                />
                <span className="shrink-0 rounded-full border border-border/55 bg-background/70 px-2 py-0.5 text-[10px] text-muted-foreground">
