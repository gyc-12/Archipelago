...
export function ImagePreview({ tab }: { tab: FileWorkspaceTab }) {
  ...
  return (
    <div className="flex h-full flex-col bg-background/70">
      {tab.loading && (
        <div className="absolute top-2 right-3 z-10 rounded-full bg-background/75 px-2.5 py-1 text-[11px] text-muted-foreground ring-1 ring-border/60 backdrop-blur-xl">
          {t("loading")}
        </div>
      )}
      {tab.content && (
        <>
          {/* Toolbar */}
          <div className="flex-none flex items-center gap-1 border-b border-border/55 bg-[rgb(255_255_255_/_78%)] px-3 py-2 backdrop-blur-2xl dark:bg-white/[0.04]">
            <button
              type="button"
              onClick={handleZoomOut}
              disabled={zoom <= ZOOM_MIN}
              className="inline-flex size-7 items-center justify-center rounded-full transition-colors hover:bg-primary/8 disabled:opacity-40"
              title={t("imageZoomOut")}
            >
              <AppleIcon name="minus" className="size-3.5" />
            </button>
            <button
              type="button"
              onClick={handleZoomReset}
              className="min-w-[3.5rem] rounded-full bg-background/70 px-2 py-1 text-center font-mono text-[11px] text-muted-foreground transition-colors hover:bg-primary/8"
            ...
            <button
              ...
              title={t("imageZoomIn")}
            >
              <AppleIcon name="plus" className="size-3.5" />
            </button>
            <button
              type="button"
              onClick={handleZoomReset}
              className="ml-0.5 inline-flex size-7 items-center justify-center rounded-full transition-colors hover:bg-primary/8"
              title={t("imageZoomReset")}
            >
              <AppleIcon name="refresh" className="size-3.5" />
            </button>
