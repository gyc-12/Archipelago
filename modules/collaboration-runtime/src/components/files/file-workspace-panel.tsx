...
function DiffFileList({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="space-y-1 border-b border-border/55 bg-[rgb(255_255_255_/_78%)] px-3 py-2 backdrop-blur-2xl dark:bg-white/[0.04]">
        <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
          ...
          <span className="rounded-full bg-green-500/10 px-2 py-0.5 font-mono text-green-700 dark:text-green-300">
            +{diffOutline.totalAdditions}
          </span>
          <span className="rounded-full bg-red-500/10 px-2 py-0.5 font-mono text-red-700 dark:text-red-300">
          ...
          {diffOutline.files.map((file) => (
            <ContextMenu key={file.key}>
              <ContextMenuTrigger asChild>
                <button
                  type="button"
                  className="group flex w-full items-center gap-2 px-3 py-1.5 text-left transition-colors hover:bg-primary/8"
                  onClick={() => {
...
export function FileWorkspacePanel() {
      ...
      .map((lineNumber) => ({
        ...
        options: {
          ...
          linesDecorationsClassName:
            "archipelago-dirty-diff-glyph archipelago-dirty-diff-added",
        },
      }))
    const modifiedDecorations = modified
      ...
      .map((lineNumber) => ({
        ...
        options: {
          isWholeLine: true,
          linesDecorationsClassName:
            "archipelago-dirty-diff-glyph archipelago-dirty-diff-modified",
        },
      }))
    const deletedDecorations = deleted
      ...
      .map((lineNumber) => ({
        ...
        options: {
          isWholeLine: false,
          linesDecorationsClassName:
            "archipelago-dirty-diff-glyph archipelago-dirty-diff-deleted",
        },
      }))
    const decorations = [
      ...addedDecorations,
  ...
  const sendSelectionToChat = useCallback(() => {
    ...
    const selectedText =
      model && selection && !selection.isEmpty()
        ...
        : renderedContent.slice(0, 2400)
    const path =
      activeFileTab.path ?? activeFileTab.description ?? activeFileTab.title
    emitAppendTextToSession({
      tabId: activeTabId,
      text: [
        "Please make a local modification to this artifact selection.",
        "",
        `Artifact: ${path}`,
        "",
        "```",
    ...
    activateConversationPane()
    toast.success("Artifact selection added to chat")
  }, [activateConversationPane, activeFileTab, activeTabId, renderedContent])
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center text-center px-6">
        <AppleIconTile
          name="fileSearch"
          tone="slate"
          className="mb-3 size-9 rounded-[11px]"
    ...
    return (
      <div className="h-full relative">
        {activeFileTab.loading && (
          <div className="absolute right-3 top-2 z-10 rounded-full bg-card/80 px-2 py-1 text-[11px] text-muted-foreground ring-1 ring-border/60 backdrop-blur-xl">
            {t("loading")}
          </div>
        )}
        {richDiffColdLoad ? (
  ...
  // Preview mode for rendered files
  const isPreviewMode =
    isFileTab &&
    activeFileTab &&
    previewFileTabIds.has(activeFileTab.id) &&
    (activeFileTab.language === "markdown" || activeFileTab.language === "html")
  ...
  if (isPreviewMode && activeFileTab?.language === "html") {
    const htmlColdLoad = activeFileTab.loading && !hasTabContent(activeFileTab)
    return (
  ...
  return (
    <div className="h-full relative">
      ...
      <div className="h-full flex flex-col min-h-0">
        {isFileTab && (
          <div className="flex items-center gap-2 border-b border-border/55 bg-[rgb(255_255_255_/_78%)] px-3 py-1.5 text-[11px] text-muted-foreground backdrop-blur-2xl dark:bg-white/[0.04]">
            <AppleIconTile
              name="file"
              tone="slate"
              className="size-6 rounded-[7px]"
            ...
            <span className="min-w-0 flex-1 truncate">
              {activeFileTab.description ?? activeFileTab.title}
            </span>
            <button
              type="button"
              onClick={sendSelectionToChat}
              className="inline-flex items-center gap-1 rounded-full border border-border/55 bg-background/75 px-2.5 py-1 text-[10px] font-medium transition-colors hover:bg-primary/8"
              title="Send selected code to chat"
            >
              <AppleIcon name="chat" className="size-3" />
              Modify
            </button>
          </div>
        )}
        {diffOutline && (
          <div className="border-b border-border/55 bg-[rgb(255_255_255_/_78%)] backdrop-blur-2xl dark:bg-white/[0.04]">
            <div className="flex items-center gap-3 px-3 py-1.5 text-[11px] text-muted-foreground">
              <AppleIconTile
                name="diff"
                tone="green"
                className="size-6 rounded-[7px]"
              ...
              {allHunks.length > 0 && (
                <div className="ml-auto flex items-center gap-1">
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-full border border-border/55 bg-background/75 px-2.5 py-1 text-[10px] transition-colors hover:bg-primary/8 disabled:opacity-40"
                  >
                    <AppleIcon name="arrowLeft" className="size-3" />
                    {t("prev")}
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    {t("next")}
                    <AppleIcon name="arrowRight" className="size-3" />
                  </button>
                </div>
              )}
            </div>
            <div className="px-2 pb-2 space-y-1 max-h-52 overflow-y-auto">
              {diffOutline.files.map((file) => {
                ...
                return (
                  <div
                    key={file.key}
                    className="rounded-[14px] border border-border/55 bg-background/70 dark:bg-white/[0.04]"
                  >
                    <button
                      type="button"
                      onClick={() => toggleFileCollapsed(file.key)}
                      className="flex w-full items-center gap-1 px-2 py-1.5 text-[11px] transition-colors hover:bg-primary/8"
                    >
                      <AppleIcon
                        name="arrowRight"
                          ...
                          return (
                            ...
                            >
                              ...
                              <button
                                ...
                                className="shrink-0 rounded-full border border-border/55 bg-background/75 px-1.5 py-0.5 transition-colors hover:bg-primary/8"
                                title={t("jumpToLine", {
                                  line: hunk.startLine,
                                })}
                              >
