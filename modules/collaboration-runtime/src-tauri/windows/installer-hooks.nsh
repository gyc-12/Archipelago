...
;
; archipelago-mcp.exe is the MCP stdio companion spawned by each agent CLI
...
; way Unix does, so stale archipelago-mcp.exe processes from a previous session
; can keep the binary file locked. The installer then fails to overwrite
; it with:
;
;     Error opening file for writing: ...\codeg\archipelago-mcp.exe
;
...
!macro NSIS_HOOK_PREINSTALL
  DetailPrint "Stopping any running archipelago-mcp processes..."
  nsExec::Exec 'taskkill /F /T /IM archipelago-mcp.exe'
  Pop $0
  ; Small grace period so the OS releases file handles before the
  ; installer attempts to overwrite archipelago-mcp.exe.
  Sleep 500
!macroend
