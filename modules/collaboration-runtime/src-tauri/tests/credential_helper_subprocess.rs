...
//! the actual `archipelago-server` binary with `--credential-helper`, pipes the
...
use archipelago_runtime_lib::db::service::app_metadata_service;
...
async fn helper_subprocess_emits_username_and_password_for_seeded_host() {
    ...
    // contention.
    {
        let db = archipelago_runtime_lib::db::init_database(&data_dir, "test")
            .await
            .expect("init db");
    ...
    // process. The helper subprocess gets `ARCHIPELAGO_DATA_DIR` via its
    ...
    let binary = env!("CARGO_BIN_EXE_archipelago-server");
    let mut child = Command::new(binary)
        .arg("--credential-helper")
        .arg("--data-dir")
        .arg(&data_dir)
        // Keep the env minimal so the subprocess uses --data-dir, not
        // whatever ARCHIPELAGO_DATA_DIR happens to be set to in the test runner.
        .env_clear()
        .env("PATH", std::env::var("PATH").unwrap_or_default())
        .env("ARCHIPELAGO_DATA_DIR", &data_dir)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn archipelago-server --credential-helper");
...
async fn helper_subprocess_outputs_nothing_for_unconfigured_host() {
    ...
    {
        let _db = archipelago_runtime_lib::db::init_database(&data_dir, "test")
            .await
            .expect("init db");
    }
