...
use archipelago_runtime_lib::db::entities::conversation;
use archipelago_runtime_lib::db::test_helpers::{fresh_in_memory_db, seed_folder};
...
async fn delegation_columns_round_trip() {
    ...
    let folder_id = seed_folder(&db, "/tmp/archipelago-delegation-test").await;
...
async fn delegation_columns_default_to_null_on_existing_create() {
    let db = fresh_in_memory_db().await;
    let folder_id = seed_folder(&db, "/tmp/archipelago-delegation-null").await;
    // The existing create helper does not set the new columns; verify they default to None.
    let conv_id = archipelago_runtime_lib::db::test_helpers::seed_conversation(
        &db,
        folder_id,
