...
mod tests {
    ...
    fn parses_gemini_session_detail_from_chat_json() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("archipelago-gemini-test-{nanos}"));
        let chats_dir = base.join("tmp").join("archipelago").join("chats");
        fs::create_dir_all(&chats_dir).expect("create chat dir");
        fs::write(
            base.join("tmp").join("archipelago").join(".project_root"),
  ...
  "messages": [
    ...
    fn parses_gemini_session_detail_from_jsonl_chat_log() {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("archipelago-gemini-jsonl-test-{nanos}"));
        let chats_dir = base.join("tmp").join("archipelago-jsonl").join("chats");
        fs::create_dir_all(&chats_dir).expect("create chat dir");
        fs::write(
            base.join("tmp")
                .join("archipelago-jsonl")
                .join(".project_root"),
...
"#;
    ...
    fn parse_detail_completion_time_uses_assistant_timestamp_not_next_message_gap() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("archipelago-gemini-completed-{nanos}"));
        let chats_dir = base.join("tmp").join("archipelago").join("chats");
        fs::create_dir_all(&chats_dir).expect("create chat dir");
