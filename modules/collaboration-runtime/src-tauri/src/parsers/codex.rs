...
mod tests {
    ...
    fn summary_title_skips_injected_messages_and_uses_real_prompt() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("archipelago-codex-test-{nanos}.jsonl"));
    ...
    fn parse_detail_sets_context_window_stats_from_token_count() {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("archipelago-codex-ctx-{nanos}.jsonl"));
    ...
    fn parse_detail_completion_time_uses_agent_message_timestamp_not_added_turn_span() {
        ...
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf =
            env::temp_dir().join(format!("archipelago-codex-completed-{nanos}.jsonl"));
    ...
    fn image_generation_end_and_call_dedupe_by_id() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf =
            env::temp_dir().join(format!("archipelago-codex-img-dedupe-{nanos}.jsonl"));
    ...
    fn image_generation_end_honors_explicit_mime_type() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf =
            env::temp_dir().join(format!("archipelago-codex-img-mime-{nanos}.jsonl"));
    ...
    fn image_generation_end_omits_revised_prompt_when_missing() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf =
            env::temp_dir().join(format!("archipelago-codex-img-noprompt-{nanos}.jsonl"));
    ...
    fn image_generation_inside_subagent_is_suppressed_in_parent() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf =
            env::temp_dir().join(format!("archipelago-codex-img-subagent-{nanos}.jsonl"));
