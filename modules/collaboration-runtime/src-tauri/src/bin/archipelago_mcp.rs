//! `archipelago-mcp` — the per-launch stdio MCP companion that an agent CLI runs
//! to surface the `delegate_to_agent` tool to its LLM.
//!
//! The agent's MCP config (injected by archipelago via `load_mcp_servers_for_agent`)
...
//! `archipelago_runtime_lib::acp::delegation::{companion, transport}` so it's
...
use archipelago_runtime_lib::acp::delegation::companion::{
    dispatch_line, drain_and_cancel_all, CompanionContext, InflightCalls, JsonRpcResponse,
    LineAction,
};
use archipelago_runtime_lib::acp::delegation::parent_watcher::{
    wait_for_parent_exit, DEFAULT_POLL_INTERVAL,
};
...
struct Args {
    ...
    socket_path: String,
    token: String,
    /// Optional PID of the archipelago / archipelago-server process that owns this
    /// session. When set, archipelago-mcp exits as soon as the parent is gone so
...
fn parse_args() -> Result<Args, String> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "archipelago-mcp --parent-connection-id <uuid> --socket-path <path> --token <secret> [--parent-pid <pid>]"
                );
                std::process::exit(0);
            }
...
async fn main() -> ExitCode {
    let args = match parse_args() {
        Ok(a) => a,
        Err(e) => {
            let _ = writeln!(std::io::stderr(), "archipelago-mcp: {e}");
        ...
        tokio::select! {
            ...
            line_result = lines.next_line() => {
                let line = match line_result {
                    ...
                    Err(e) => {
                        let _ = writeln!(std::io::stderr(), "archipelago-mcp: read stdin: {e}");
                ...
                match action {
                    LineAction::Respond(resp) => {
                        if let Err(e) = write_response(&stdout, &resp).await {
                            let _ = writeln!(std::io::stderr(), "archipelago-mcp: write stdout: {e}");
                            return ExitCode::from(1);
                        }
                    }
                        ...
                        tokio::spawn(async move {
                            if let Some(resp) = spawned.future.await {
                                if let Err(e) = write_response(&stdout, &resp).await {
                                    let _ = writeln!(
                                        std::io::stderr(),
                                        "archipelago-mcp: write stdout: {e}"
                                    );
                                }
                            }
                        });
