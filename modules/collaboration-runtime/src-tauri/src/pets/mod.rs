...
//! `paths::archipelago_pets_root()` and is **decoupled from Tauri** so the same
...
use crate::paths::archipelago_pets_root;
...
fn pet_dir(id: &str) -> Result<PathBuf, AppCommandError> {
    validate_pet_id(id)?;
    Ok(archipelago_pets_root().join(id))
}
...
pub(crate) fn ensure_pets_root_or_create() -> Result<PathBuf, AppCommandError> {
    let root = archipelago_pets_root();
    if !root.exists() {
...
mod tests {
    ...
    // Filesystem-touching tests rely on `ARCHIPELAGO_HOME`, which is shared global
