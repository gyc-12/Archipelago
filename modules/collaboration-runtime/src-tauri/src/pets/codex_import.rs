//! One-way import: Codex pets → archipelago pets.
//!
//! Codex stores pets at `${CODEX_HOME:-~/.codex}/pets/<id>/` with the same
//! `pet.json` + `spritesheet.webp` layout archipelago uses. We copy the directory
...
/// Copy selected Codex pets into `~/.archipelago/pets/`. Each entry copies the full
        ...
        let target_id = if pet.already_imported {
            if !request.overwrite_with_suffix {
                skipped.push(ImportSkipped {
                    ...
                    reason: "Pet with this id already exists in archipelago.".to_string(),
                });
                continue;
            }
