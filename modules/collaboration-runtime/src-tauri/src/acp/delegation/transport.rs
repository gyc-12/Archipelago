//! Wire format for `archipelago-mcp` companion ↔ main process round-trip over UDS
...
//! ### Version coupling
//!
//! The companion (`archipelago-mcp`) and the listener (inside the archipelago main
...
//! `archipelago-mcp` uses an absolute path that is replaced atomically by the
...
pub struct BrokerRequest {
    ...
    /// anything else.
    pub token: String,
    /// archipelago-internal ACP connection UUID for the parent session.
...
mod tests {
    ...
    async fn uds_round_trip() {
        ...
        let path = dir.path().join("archipelago-mcp.sock");
