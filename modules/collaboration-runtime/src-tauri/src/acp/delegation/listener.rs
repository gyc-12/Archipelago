//! Main-process side of the `archipelago-mcp` round-trip: accept UDS / named-pipe
...
/// archipelago instances on the same machine don't collide.
///
/// Unix: a `.sock` file inside `temp_dir`.
/// Windows: a named pipe address `\\.\pipe\archipelago-delegation-<pid>`. Windows
