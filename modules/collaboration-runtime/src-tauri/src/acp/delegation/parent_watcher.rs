//! Self-cleanup watchdog for `archipelago-mcp`.
//!
...
//! (agent CLI that hangs, parent archipelago crash that orphans the agent) can
//! leave `archipelago-mcp` running forever, holding open the binary file and a
//! companion connection that no one will ever read from.
//!
//! When the parent archipelago / archipelago-server passes `--parent-pid <pid>` on
//! the command line, `archipelago-mcp` spawns this watchdog. It polls the OS
...
//! invisible next to the other work `archipelago-mcp` does.
//!
...
/// Default polling cadence. Fast enough that a stale `archipelago-mcp.exe`
...
/// permission glitch can't cause the watchdog to kill `archipelago-mcp` while
