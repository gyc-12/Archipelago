//! Companion-side MCP protocol — the bits that live inside the `archipelago-mcp`
...
/// won't notice the bound being hit. Misses are absorbed by the archipelago
...
/// Static MCP tool schema. Lives next to this module so archipelago-mcp ships
...
/// can't pin shutdown — the archipelago main side's `cancel_by_parent` cascade
...
mod tests {
    ...
    fn ctx() -> CompanionContext {
        CompanionContext {
            ...
            socket_path: "/tmp/archipelago-mcp-companion-test-nope.sock".into(),
            token: "tok".into(),
        }
    }
    ...
    async fn initialize_returns_protocol_version() {
        ...
        assert_eq!(result["serverInfo"]["name"], "archipelago-mcp");
    }
