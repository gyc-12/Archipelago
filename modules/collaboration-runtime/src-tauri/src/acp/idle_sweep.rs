...
/// `ARCHIPELAGO_ACP_IDLE_TIMEOUT_SECS`. The sweep only runs against
...
/// Read the idle timeout from `ARCHIPELAGO_ACP_IDLE_TIMEOUT_SECS`, falling back
...
pub fn idle_timeout_from_env() -> Option<Duration> {
    let secs = match std::env::var("ARCHIPELAGO_ACP_IDLE_TIMEOUT_SECS") {
...
mod tests {
    ...
    /// runs tests in parallel by default; setting `ARCHIPELAGO_ACP_IDLE_TIMEOUT_SECS`
    /// in concurrent tests would interleave with each other.
    #[test]
    fn idle_timeout_env_parsing() {
        // Disabled when zero.
        std::env::set_var("ARCHIPELAGO_ACP_IDLE_TIMEOUT_SECS", "0");
        assert!(idle_timeout_from_env().is_none());

        // Falls back to default when unparseable.
        std::env::set_var("ARCHIPELAGO_ACP_IDLE_TIMEOUT_SECS", "not-a-number");
        assert_eq!(
        ...
        std::env::set_var("ARCHIPELAGO_ACP_IDLE_TIMEOUT_SECS", "120");
        assert_eq!(idle_timeout_from_env().unwrap().as_secs(), 120);

        // Falls back to default when unset.
        std::env::remove_var("ARCHIPELAGO_ACP_IDLE_TIMEOUT_SECS");
        assert_eq!(
            idle_timeout_from_env().unwrap().as_secs(),
            DEFAULT_IDLE_TIMEOUT_SECS
        );
