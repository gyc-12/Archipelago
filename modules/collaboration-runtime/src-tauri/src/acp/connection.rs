...
const INIT_TIMEOUT_SENTINEL: &str = "__archipelago_init_timeout__";
...
async fn build_agent(
    ...
    match meta.distribution {
        ...
        AgentDistribution::Binary {
            ...
            //   the `ARCHIPELAGO_ACP_DEBUG=1` env var so production builds
            ...
            let stdio_debug_enabled = std::env::var("ARCHIPELAGO_ACP_DEBUG")
...
pub async fn spawn_agent_connection(
    ...
    // Forward only the archipelago git credential helper keys into the terminal
...
/// Context the connection layer needs to inject the built-in `archipelago-delegate`
...
/// Locate the `archipelago-mcp` companion binary across the supported deployment
/// shapes:
///
/// 1. `ARCHIPELAGO_MCP_BIN` env override — explicit absolute path. Lets dev shells,
...
///    shipping target. Tauri sidecar (`Contents/MacOS/archipelago-mcp` on macOS,
///    next to `archipelago.exe` on Windows, next to the unix binary on Linux
///    deb/rpm), `install.sh`/`install.ps1` (drops `archipelago-mcp` next to
///    `archipelago-server`), Docker image (`/usr/local/bin/archipelago-mcp` next to
///    `archipelago-server`), and `cargo build` dev output
///    (`target/<profile>/archipelago-mcp`).
...
fn locate_archipelago_mcp_binary() -> Option<PathBuf> {
    ...
    if let Some(raw) = std::env::var_os("ARCHIPELAGO_MCP_BIN") {
...
/// Append the built-in `archipelago-delegate` MCP entry if delegation is enabled
...
/// `delegate_to_agent`, which is the right degradation when archipelago-mcp didn't
/// make it into the install.
async fn inject_archipelago_delegate_mcp(
    ...
    let Some(binary_path) = locate_archipelago_mcp_binary() else {
        eprintln!(
            "[delegation][WARN] archipelago-mcp companion binary not found (checked ARCHIPELAGO_MCP_BIN, \
             exe sibling, and PATH); skipping delegate_to_agent tool injection for \
             connection {parent_connection_id}. Reinstall Archipelago or set ARCHIPELAGO_MCP_BIN to fix."
        );
        return None;
    };
    ...
    injection
        ...
        .await;
    let mut server = McpServerStdio::new("archipelago-delegate", binary_path);
    server = server.args(vec![
        ...
        "--token".to_string(),
        token.clone(),
        // Self-cleanup watchdog: archipelago-mcp exits when this PID is gone so
...
async fn run_connection(
    ...
    Client
        ...
        .connect_with(agent, async move |cx| -> Result<(), sacp::Error> {
                ...
                Err(_) => {
                    eprintln!(
                        "[ACP][{agent_name_for_log}] Initialize TIMED OUT after {:?} \
                         ...
                         JSON-RPC trace, re-launch with ARCHIPELAGO_ACP_DEBUG=1.",
                        init_started.elapsed()
                    );
            ...
            // Inject the built-in `archipelago-delegate` MCP server. Stdio is
            ...
            let delegate_token = if let Some(inj) = delegation_injection.as_ref() {
                inject_archipelago_delegate_mcp(&mut mcp_servers, inj, &conn_id, &cwd).await
            } else {
                None
            };
...
mod tests {
    ...
    fn build_new_session_request_sets_claude_raw_meta() {
        let cwd = std::path::PathBuf::from("/tmp/archipelago");
    ...
    // ─── inject_archipelago_delegate_mcp: enabled=false short-circuit ──────────
    //
    // Guards the broker-level kill switch: when the broker config has
    // `enabled: false`, the
    ...
    // `inject_archipelago_delegate_mcp` is the single chokepoint that keeps a
    // archipelago-delegate stdio MCP out of every ACP session after delegation
    // is explicitly disabled.
    #[tokio::test]
    async fn inject_archipelago_delegate_skipped_when_broker_disabled() {
        ...
        // No set_config call: broker carries its low-level default config,
        // which is `enabled: false` until the product settings layer applies
        // persisted/default settings during app bootstrap.
        let injection = DelegationInjection {
            broker,
            tokens: Arc::new(TokenRegistry::default()),
            socket_path: std::path::PathBuf::from("/tmp/archipelago-delegate.sock"),
        };

        let mut servers: Vec<McpServer> = Vec::new();
        let result = inject_archipelago_delegate_mcp(
            &mut servers,
            &injection,
            "parent-conn",
