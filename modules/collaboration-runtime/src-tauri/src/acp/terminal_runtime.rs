...
pub struct TerminalRuntime {
    ...
    /// the agent's per-request `env` is applied. This is where the archipelago
...
impl TerminalRuntime {
    ...
    pub async fn create_terminal(
        ...
        // archipelago credential helper), then layer the agent's request env on
...
mod tests {
    ...
    /// layer with the archipelago credential helper's `GIT_CONFIG_*` keys —
    ...
    async fn base_env_propagates_and_request_env_overrides() {
        ...
        base_env.insert(
            "ARCHIPELAGO_TEST_BASE_VAR".to_string(),
            "from_base".to_string(),
        );
        base_env.insert("ARCHIPELAGO_TEST_OVERRIDE".to_string(), "loses".to_string());
        ...
        request.args = vec![
            ...
            "printf '%s\\n' \"$ARCHIPELAGO_TEST_BASE_VAR\" \"$ARCHIPELAGO_TEST_OVERRIDE\"".into(),
        ];
        request.env = vec![EnvVariable::new(
            "ARCHIPELAGO_TEST_OVERRIDE",
            "request_wins",
        )];
