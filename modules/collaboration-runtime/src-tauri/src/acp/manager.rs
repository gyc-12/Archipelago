...
/// Read the spawn-handshake timeout from `ARCHIPELAGO_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS`,
...
fn spawn_handshake_timeout_from_env() -> Duration {
    let secs = std::env::var("ARCHIPELAGO_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS")
        .ok()
...
pub struct ConnectionManager {
    ...
    /// `archipelago-delegate` appear in the agent's MCP server list during ACP
...
impl ConnectionManager {
    ...
    pub async fn spawn_agent(
        ...
        // latencies and tune `ARCHIPELAGO_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS`.
    ...
    pub async fn emit_group_collaboration_plan(
        &self,
        conn_id: &str,
        plan: crate::acp::group_collaboration::GroupCollaborationPlan,
    ) -> Result<(), AcpError> {
        let (state_arc, emitter) = {
            let connections = self.connections.lock().await;
            let conn = connections
                .get(conn_id)
                .ok_or_else(|| AcpError::ConnectionNotFound(conn_id.into()))?;
        ...
        emit_with_state(
            ...
            AcpEvent::GroupCollaborationPlan {
                ...
                requested_mentions: plan.requested_mentions,
                invalid_mentions: plan.invalid_mentions,
    ...
    /// what `archipelago-mcp` will pass through to `session/set_config_option`
    ...
    pub async fn list_connections(&self) -> Vec<ConnectionInfo> {
        ...
        for (id, agent_type, state) in values {
...
/// fail any git command that depends on the archipelago-injected helper.
#[derive(Clone)]
...
mod tests {
    ...
    #[tokio::test]
    async fn list_connections_reports_live_session_state_status() {
        let mgr = ConnectionManager::new();
        let mut conn = fake_connection("c1", None);
    ...
    fn spawn_handshake_timeout_from_env_uses_default_when_unset() {
        ...
        let prev = std::env::var("ARCHIPELAGO_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS").ok();
        std::env::remove_var("ARCHIPELAGO_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS");
        let default = spawn_handshake_timeout_from_env();
        assert_eq!(default, Duration::from_secs(SPAWN_HANDSHAKE_TIMEOUT_SECS));

        std::env::set_var("ARCHIPELAGO_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS", "5");
        assert_eq!(spawn_handshake_timeout_from_env(), Duration::from_secs(5));

        std::env::set_var("ARCHIPELAGO_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS", "garbage");
        assert_eq!(
            spawn_handshake_timeout_from_env(),
        ...
        // Restore.
        match prev {
            Some(v) => std::env::set_var("ARCHIPELAGO_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS", v),
            None => std::env::remove_var("ARCHIPELAGO_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS"),
        }
    }
