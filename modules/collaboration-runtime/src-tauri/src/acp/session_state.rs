...
pub struct ToolCallState {
    ...
    ///
    /// Convention used by archipelago's multi-agent delegation (the `delegate_to_agent`
    /// MCP tool) — `DelegationBroker` writes the following object under
    /// `meta["archipelago.delegation"]` on the parent's active tool call:
    ///
    /// ```jsonc
    /// {
...
pub struct SessionState {
    ...
    /// `TokenRegistry` when `archipelago-delegate` is injected at init.
    ...
    pub fn apply_event(&mut self, payload: &AcpEvent) {
        match payload {
            ...
            | AcpEvent::DelegationCompleted { .. } => {
                ...
                // (key `archipelago.delegation`), updated by the ToolCall /
