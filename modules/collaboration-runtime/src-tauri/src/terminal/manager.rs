...
/// flags and the `eval "$ARCHIPELAGO_CMD"` wrapping for shells we know speak that
        ...
        match detect_windows_shell_flavor(shell) {
            WindowsShellFlavor::Cmd => {
                if let Some(command) = initial_command {
                    cmd.env("ARCHIPELAGO_CMD", command);
                    // Set UTF-8 code page before running the actual command
                    cmd.args(["/D", "/S", "/C", "chcp 65001 >nul & %ARCHIPELAGO_CMD%"]);
                } else {
            ...
            WindowsShellFlavor::PowerShell => {
                if let Some(command) = initial_command {
                    ...
                    cmd.args([
                        ...
                        "-NoProfile",
                        "-Command",
                        "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $ErrorActionPreference = 'Stop'; Invoke-Expression $env:ARCHIPELAGO_CMD",
                    ]);
                } else {
                    ...
                    cmd.args([
                        ...
                        "-NoExit",
                        "-Command",
                        "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $host.UI.RawUI.WindowTitle = 'Archipelago'",
                    ]);
                }
            }
            WindowsShellFlavor::Posix => {
                ...
                cmd.env("TERM_PROGRAM", "archipelago");
                cmd.env("LANG", "C.UTF-8");
                if let Some(command) = initial_command {
                    cmd.env("ARCHIPELAGO_CMD", command);
                    cmd.args(["-l", "-i", "-c", "eval \"$ARCHIPELAGO_CMD\""]);
                } else {
                    cmd.args(["-l", "-i"]);
                }
            }
