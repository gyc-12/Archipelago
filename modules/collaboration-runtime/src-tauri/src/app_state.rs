...
pub struct AppState {
    ...
    /// `archipelago-delegate` MCP entry, revoked on parent teardown.
    pub delegation_tokens: Arc<TokenRegistry>,
    /// Absolute path of the UDS / named pipe the companion connects to.
    /// PID-scoped so multiple archipelago processes on the same host don't fight.
...
/// path. Shared between archipelago-server bootstrap and the Tauri `setup` block
