...
mod tests {
    ...
    async fn list_all_excludes_children_by_default() {
        ...
        let folder = seed_folder(&db, "/tmp/archipelago-list-children-default").await;
    ...
    async fn list_all_includes_children_when_requested() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/archipelago-list-children-on").await;
    ...
    async fn list_children_returns_only_matching_parent() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/archipelago-list-children-only").await;
    ...
    async fn list_children_excludes_soft_deleted() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/archipelago-list-children-soft-del").await;
        let (parent, child) = seed_parent_with_child(&db.conn, folder).await;
