...
pub async fn remove_folder_from_workspace(
    ...
    let deleted_group_ids =
        crate::commands::groups::delete_groups_by_folder_core(&db, folder_id).await?;
    let emitter = EventEmitter::Tauri(app);
    for group_id in deleted_group_ids {
        crate::web::event_bridge::emit_event(
            &emitter,
            crate::commands::groups::GROUP_DELETED_EVENT,
            &crate::commands::groups::GroupDeletedPayload {
                group_id,
...
pub async fn clone_repository(
    ...
    // `ARCHIPELAGO_DATA_DIR` reaches the git credential helper invoked by
    // this subprocess.
...
mod tests {
    ...
    async fn add_folder_to_history_core_derives_name_from_path() {
        ...
        let entry = add_folder_to_history_core(&db, "/tmp/archipelago-test-project".into())
            .await
            .expect("add folder");
        assert_eq!(entry.name, "archipelago-test-project");
        assert_eq!(entry.path, "/tmp/archipelago-test-project");
    }

    #[tokio::test]
    async fn add_folder_to_history_core_upserts_on_duplicate_path() {
        let db = fresh_in_memory_db().await;
        let path = "/tmp/archipelago-dup-test".to_string();
    ...
    async fn remove_folder_from_history_core_soft_deletes() {
        let db = fresh_in_memory_db().await;
        let path = "/tmp/archipelago-remove-test".to_string();
    ...
    async fn update_folder_color_core_roundtrips() {
        let db = fresh_in_memory_db().await;
        let entry = add_folder_to_history_core(&db, "/tmp/archipelago-color-test".into())
            .await
            .expect("add");
    ...
    async fn update_folder_default_agent_core_set_then_clear() {
        let db = fresh_in_memory_db().await;
        let entry = add_folder_to_history_core(&db, "/tmp/archipelago-agent-test".into())
            .await
            .expect("add");
