...
const MARKETPLACE_USER_AGENT: &str = "archipelago-mcp-market/1.0";
...
/// The marketplace suffix archipelago uses when toggling user-scope Claude Code
...
fn parse_json_text<T: DeserializeOwned>(raw: &str, context: &str) -> Result<T, AppCommandError> {
...
async fn parse_json_response<T: DeserializeOwned>(
    ...
    parse_json_text(&raw, context)
}

async fn fetch_marketplace_json_with_curl<T: DeserializeOwned>(
    url: &str,
    context: &str,
) -> Result<T, AppCommandError> {
    ...
    let output = tokio::process::Command::new(curl)
        ...
        .arg("20")
        .arg("--header")
        .arg(format!("User-Agent: {MARKETPLACE_USER_AGENT}"))
        .arg("--header")
        ...
        .output()
        .await
        .map_err(|e| mcp_network(format!("{context}: curl fallback failed to start: {e}")))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        let message = stderr.trim();
        return Err(mcp_network(if message.is_empty() {
            format!(
                "{context}: curl fallback failed with status {}",
                output.status
            )
        } else {
            format!(
                "{context}: curl fallback failed with status {}: {message}",
                output.status
            )
        }));
    }
    ...
    let raw = String::from_utf8(output.stdout).map_err(|e| {
        mcp_network(format!(
            "{context}: curl fallback returned non-UTF8 body: {e}"
        ))
    })?;
    parse_json_text(&raw, context)
}

async fn fetch_marketplace_json<T, F>(
    request_context: &str,
    status_context: &str,
    parse_context: &str,
    url: &str,
...
where
    ...
    F: FnMut() -> reqwest::RequestBuilder,
{
    let response = send_request_with_retry(request_context, build).await?;
    let status = response.status();

    if status.is_success() {
        return parse_json_response::<T>(response, parse_context).await;
    }

    if status == reqwest::StatusCode::FORBIDDEN {
        eprintln!("[MCP] {request_context}: reqwest returned HTTP 403; retrying via curl fallback");
        return fetch_marketplace_json_with_curl(url, parse_context).await;
    }

    Err(mcp_network(format!("{status_context}: HTTP {status}")))
}
...
async fn search_official_registry(
    ...
    let url = format!(
        "https://registry.modelcontextprotocol.io/v0.1/servers?limit={limit}&version=latest&search={}",
        urlencoding::encode(trimmed)
    );

    let payload = fetch_marketplace_json::<Value, _>(
...
async fn fetch_official_server_detail(
    ...
    let client = marketplace_http_client()?;
    fetch_marketplace_json::<OfficialServerResponse, _>(
        "failed to fetch official MCP server detail",
        "official MCP server detail request failed",
...
async fn search_smithery(
    ...
    let trimmed = query.trim();
    let url = format!(
        "https://api.smithery.ai/servers?limit={limit}&q={}",
        urlencoding::encode(trimmed)
    );

    let payload = fetch_marketplace_json::<SmitheryServerListResponse, _>(
        "failed to query smithery marketplace",
        "smithery marketplace request failed",
...
async fn fetch_smithery_server_summary(
    ...
    let client = marketplace_http_client()?;
    let url = format!(
        "https://api.smithery.ai/servers?limit=30&q={}",
        urlencoding::encode(server_id)
    );

    let payload = fetch_marketplace_json::<SmitheryServerListResponse, _>(
        "failed to fetch smithery server summary",
        "smithery server summary request failed",
...
async fn fetch_smithery_server_detail(
    ...
    let encoded_id = urlencoding::encode(server_id);
    let url = format!("https://api.smithery.ai/servers/{encoded_id}");
    let client = marketplace_http_client()?;
    fetch_marketplace_json::<SmitheryServerDetail, _>(
        "failed to fetch smithery server detail",
        "smithery server detail request failed",
        "failed to parse smithery server detail",
        &url,
        || client.get(url.clone()),
    )
    .await
