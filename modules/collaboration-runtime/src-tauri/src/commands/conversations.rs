...
use crate::db::service::{
    conversation_service, folder_service, group_service, import_service, tab_service,
};
...
/// Build the `meta["archipelago.delegation"]` value for a delegation child loaded
...
/// `meta["archipelago.delegation"]` to the DB-derived snapshot. Skips blocks
...
/// MCP-prefixed (`mcp__archipelago-delegate__delegate_to_agent`) and bare forms
            ...
            } = block
            {
                ...
                if let Some(child) = by_parent_tool_use_id.get(tu.as_str()) {
                    *meta = Some(serde_json::json!({
                        "archipelago.delegation": build_historical_delegation_meta(child),
                    }));
                }
            }
        }
...
pub async fn get_folder_conversation_core(
    ...
    // don't carry `meta["archipelago.delegation"]`, so a reloaded conversation
...
pub async fn create_conversation_with_group_agent_core(
    conn: &sea_orm::DatabaseConnection,
    folder_id: i32,
    agent_type: AgentType,
    title: Option<String>,
) -> Result<(i32, Vec<GroupAgentInfo>), AppCommandError> {
    let conversation_id = create_conversation_core(conn, folder_id, agent_type, title).await?;
    let mut agents = Vec::new();
    let working_dir = folder_service::get_folder_by_id(conn, folder_id)
        .await
        .map_err(AppCommandError::from)?
        .map(|folder| folder.path)
        .unwrap_or_default();
    let agent_type_value = serde_json::to_value(agent_type)
        .ok()
        .and_then(|v| v.as_str().map(String::from))
        .unwrap_or_default();
    for group in group_service::list_groups_by_folder(conn, folder_id)
        .await
        .map_err(AppCommandError::from)?
    {
        let agent = group_service::add_agent(
            ...
            group_service::DEFAULT_GROUP_AGENT_ROLE.to_string(),
        ...
        .await
        .map_err(AppCommandError::from)?;
        agents.push(GroupAgentInfo::from(agent));
    }
    Ok((conversation_id, agents))
}
...
pub async fn create_conversation(
    ...
    let (conversation_id, agents) =
        create_conversation_with_group_agent_core(&db.conn, folder_id, agent_type, title).await?;
    let emitter = crate::web::event_bridge::EventEmitter::Tauri(app);
    for agent in agents {
        crate::web::event_bridge::emit_event(
            &emitter,
            crate::commands::groups::AGENT_UPSERTED_EVENT,
            &agent,
        );
    }
    Ok(conversation_id)
...
pub async fn delete_conversation_with_group_agents_core(
    conn: &sea_orm::DatabaseConnection,
    conversation_id: i32,
) -> Result<Vec<GroupAgentInfo>, AppCommandError> {
    let agents = group_service::list_agents_by_conversation(conn, conversation_id)
        .await
        .map_err(AppCommandError::from)?;
    delete_conversation_core(conn, conversation_id).await?;
    for agent in &agents {
        group_service::remove_agent(conn, agent.id)
            .await
            .map_err(AppCommandError::from)?;
    }
    Ok(agents.into_iter().map(GroupAgentInfo::from).collect())
}
...
pub async fn delete_conversation(
    ...
    let agents = delete_conversation_with_group_agents_core(&db.conn, conversation_id).await?;
    let emitter = crate::web::event_bridge::EventEmitter::Tauri(app);
    for agent in agents {
        crate::web::event_bridge::emit_event(
            &emitter,
            crate::commands::groups::AGENT_DELETED_EVENT,
            &crate::commands::groups::AgentDeletedPayload {
                group_id: Some(agent.group_id),
                agent_id: agent.id,
            },
        );
    }
...
mod tests {
    ...
    fn inject_delegation_meta_preserves_live_broker_meta() {
        ...
        let pre_existing = serde_json::json!({ "archipelago.delegation": { "status": "running", "child_conversation_id": 999 } });
    ...
    async fn get_folder_conversation_core_injects_meta_for_real_child() {
        ...
        let folder_id = seed_folder(&db, "/tmp/archipelago-inject-test").await;
    ...
    async fn create_conversation_core_happy_path() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/archipelago-conv-test-1").await;
    ...
    #[tokio::test]
    async fn conversation_group_agent_sync_roundtrip() {
        use crate::db::service::group_service;

        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/archipelago-group-agent-sync").await;
        let group = group_service::create_group(
            &db.conn,
            "Island Group".into(),
            Some(folder_id),
            Some("/tmp/archipelago-group-agent-sync".into()),
        )
        .await
        .expect("create group");

        let (conversation_id, agents) = create_conversation_with_group_agent_core(
            &db.conn,
            folder_id,
            AgentType::Codex,
            Some("Archipelago Codex".into()),
        )
        .await
        .expect("create conversation with agent binding");

        assert_eq!(agents.len(), 1);
        assert_eq!(agents[0].group_id, group.id);
        assert_eq!(agents[0].conversation_id, Some(conversation_id));
        assert_eq!(agents[0].agent_type, "codex");
        assert_eq!(agents[0].role, group_service::DEFAULT_GROUP_AGENT_ROLE);

        let active_agents = group_service::list_agents_by_group(&db.conn, group.id)
            .await
            .expect("list agents");
        assert_eq!(active_agents.len(), 1);

        let deleted_agents = delete_conversation_with_group_agents_core(&db.conn, conversation_id)
            .await
            .expect("delete conversation with agent binding");
        assert_eq!(deleted_agents.len(), 1);
        assert_eq!(deleted_agents[0].id, active_agents[0].id);

        let remaining_agents = group_service::list_agents_by_group(&db.conn, group.id)
            .await
            .expect("list remaining agents");
        assert!(remaining_agents.is_empty());
    }

    #[tokio::test]
    async fn save_opened_tabs_core_roundtrip() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/archipelago-tabs-test").await;
        let items = vec![
            OpenedTab {
                id: 0,
                folder_id,
    ...
    async fn update_conversation_status_core_invalid_string_errors() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/archipelago-status-test").await;
    ...
    async fn update_conversation_title_core_roundtrip() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/archipelago-title-test").await;
    ...
    async fn delete_conversation_core_soft_deletes() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/archipelago-delete-test").await;
    ...
    async fn list_child_conversations_core_returns_empty_for_no_parent() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/archipelago-list-children-empty").await;
    ...
    async fn list_child_conversations_core_returns_only_matching_children() {
        ...
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/archipelago-list-children-match").await;
