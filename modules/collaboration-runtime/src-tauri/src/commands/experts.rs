...
//!
//! Experts are curated skills (from obra/superpowers) that Archipelago bundles
//! into its binary via `include_dir!`. On startup they are extracted to a
//! central directory `~/.archipelago/skills/<id>/`. Users can then enable an
...
//! database state, and updates propagate automatically when Archipelago upgrades
...
const CENTRAL_DIR_NAME: &str = ".archipelago";
...
fn classify_link(link_path: &Path, expected_target: &Path) -> ExpertLinkState {
    ...
    match (resolved_link, resolved_expected) {
        ...
        (Some(l), Some(e)) if paths_equivalent(&l, &e) => ExpertLinkState::LinkedToArchipelago,
...
fn ensure_central_experts_installed_blocking() -> InstallReport {
    ...
    manifest.archipelago_version = env!("CARGO_PKG_VERSION").to_string();
...
pub async fn experts_list_for_agent(
    ...
    for meta in meta_list {
        ...
        let is_linked = dirs.iter().any(|dir| {
            ...
            classify_link(&candidate, &central_path) == ExpertLinkState::LinkedToArchipelago
        });
        if !is_linked {
            continue;
        }
    ...
    match create_link_raw(&central, &link_path) {
        ...
        Err(err) if err.kind() == io::ErrorKind::AlreadyExists => {
            ...
            match classify_link(&link_path, &central) {
                ExpertLinkState::LinkedToArchipelago => {
                    // Idempotent success.
                }
...
pub async fn experts_unlink_from_agent(
    ...
    for dir in dirs {
        ...
        if matches!(
            state,
            ExpertLinkState::LinkedToArchipelago | ExpertLinkState::Broken
        ) {
