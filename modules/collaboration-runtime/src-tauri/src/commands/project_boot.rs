...
fn validate_project_name(name: &str) -> Result<(), AppCommandError> {
    ...
    // "..\\x" as traversal — reject separators explicitly since archipelago ships on
    // Windows too.
...
/// archipelago's six supported agents, mapped to the `skills` CLI's `--agent` ids.
/// The launcher only ever targets these agents (never the CLI's full 55+ agent
/// universe via `--all`/`--agent '*'`), so installs stay scoped to tools archipelago
...
/// speak) to archipelago's internal `AgentType`, which drives skill-dir resolution.
...
/// directories. Reuses archipelago's own skill-dir resolution + enumeration
