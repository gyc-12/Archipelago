...
//!
//! When a desktop window is opened against a remote archipelago-server, every API
...
/// `sec-websocket-protocol` header looking for `archipelago-token.{base64url}`.
const WS_EVENT_PROTOCOL: &str = "archipelago-events";
const WS_TOKEN_PROTOCOL_PREFIX: &str = "archipelago-token.";
...
/// Forward an HTTP API call to the remote archipelago-server identified by
...
pub async fn remote_http_call(
    ...
    if !status.is_success() {
        ...
        // The remote archipelago-server always returns `Json(AppCommandError)`
...
/// remote archipelago-server identified by `connection_id`. Sibling of
...
//
// Issue #179 follow-up: a Tauri client bound to a remote archipelago-server
...
fn partial_download_path(save_path: &str, transfer_id: &str) -> String {
    format!("{save_path}.archipelago-download-{transfer_id}.part")
}
...
/// this connection. The remote archipelago-server's `ws.rs` emits frames shaped
...
/// `archipelago-token.{base64url(token)}`. The latter is what browser
...
mod tests {
    ...
    fn partial_download_path_is_unique_sibling() {
        ...
        assert_eq!(path, "/tmp/out.zip.archipelago-download-abc.part");
    }
