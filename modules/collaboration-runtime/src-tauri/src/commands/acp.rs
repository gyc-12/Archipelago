...
const CODEX_DEFAULT_MODEL_PROVIDER: &str = "archipelago";
...
/// (`~/.archipelago/npm-global/`) so packages installed via the EACCES fallback are
/// found as well.
...
/// Fallback: install an npm package into a user-local prefix (`~/.archipelago/npm-global/`)
...
/// Uninstall an npm package from the user-local prefix (`~/.archipelago/npm-global/`).
...
fn persist_codex_local_config(config_patch_json: Option<&str>) -> Result<(), AcpError> {
    ...
    if provider_name == CODEX_DEFAULT_MODEL_PROVIDER {
        provider_table.insert(
            "name".to_string(),
            toml::Value::String(CODEX_DEFAULT_MODEL_PROVIDER.to_string()),
        );
        provider_table.insert(
            "wire_api".to_string(),
...
/// the central `~/.archipelago/skills/` store.
...
pub(crate) fn build_runtime_env_from_setting(
    ...
    let mut merged = setting
        ...
        .unwrap_or_default();
    remove_empty_runtime_env_values(agent_type, &mut merged);
    merged
}

fn remove_empty_runtime_env_values(
    agent_type: AgentType,
    runtime_env: &mut BTreeMap<String, String>,
) {
    runtime_env.retain(|_, value| !value.trim().is_empty());

    if agent_type == AgentType::ClaudeCode {
        // Claude's native `settings.json` supports both exact env names and
        // `*_NAME` aliases for display/config purposes. Only the exact
        // runtime env keys should reach ACP child processes.
        runtime_env.retain(|key, _| !key.starts_with("ANTHROPIC_") || !key.ends_with("_NAME"));
    }
}

fn apply_model_provider_values_to_runtime_env(
    agent_type: AgentType,
    api_url: &str,
    api_key: &str,
    model: Option<&str>,
    ...
    if !api_url.trim().is_empty() {
        runtime_env.insert(url_key.to_string(), api_url.to_string());
    }
    if !api_key.trim().is_empty() {
        runtime_env.insert(key_key.to_string(), api_key.to_string());
    }

    for (key, value) in parse_provider_model(agent_type, model) {
        match value {
            Some(value) => {
...
pub(crate) async fn apply_model_provider_env(
    ...
    apply_model_provider_values_to_runtime_env(
        agent_type,
        &provider.api_url,
        &provider.api_key,
...
pub(crate) async fn build_session_runtime_env(
    ...
    let mut runtime_env = build_runtime_env_from_setting(agent_type, setting.as_ref());
...
pub async fn acp_connect(
    ...
    // Resolve through the effective data dir so a custom `ARCHIPELAGO_DATA_DIR`
...
pub async fn acp_prompt(
    ...
    collaboration_mode: Option<crate::acp::group_collaboration::GroupCollaborationMode>,
    db: State<'_, crate::db::AppDatabase>,
    manager: State<'_, ConnectionManager>,
) -> Result<(), AcpError> {
    let enrichment = crate::acp::group_collaboration::analyze_group_collaboration_prompt_with_mode(
        &db,
        blocks,
        conversation_id,
    ...
    if let Some(plan) = enrichment.plan {
        manager
            .emit_group_collaboration_plan(&connection_id, plan)
            .await?;
    }
    manager
        .send_prompt_linked(
...
/// archipelago-mcp later spawns a subagent.
///
...
mod tests {
    ...
    fn unique_test_dir(name: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("archipelago-acp-{name}-{}", uuid::Uuid::new_v4()));
    ...
    #[test]
    fn model_provider_runtime_env_applies_claude_models() {
        let mut env = BTreeMap::new();
        env.insert("ANTHROPIC_MODEL".to_string(), "stale-main".to_string());
        env.insert(
            "ANTHROPIC_DEFAULT_OPUS_MODEL".to_string(),
            "stale-opus".to_string(),
        );
    ...
    #[test]
    fn runtime_env_uses_only_archipelago_agent_setting_env() {
        let now = chrono::Utc::now();
        let env_json = serde_json::json!({
            "ANTHROPIC_AUTH_TOKEN": "token-from-archipelago-settings",
            "ANTHROPIC_BASE_URL": "https://anthropic.example/api",
            "ANTHROPIC_MODEL": "claude-sonnet-4-6",
            "ANTHROPIC_DEFAULT_OPUS_MODEL_NAME": "display-only-opus",
            "EMPTY_VALUE": " "
        })
        .to_string();
        ...
        let env = build_runtime_env_from_setting(AgentType::ClaudeCode, Some(&setting));

        assert_eq!(
            env.get("ANTHROPIC_AUTH_TOKEN").map(String::as_str),
            Some("token-from-archipelago-settings")
        );
        assert_eq!(
            env.get("ANTHROPIC_BASE_URL").map(String::as_str),
            Some("https://anthropic.example/api")
        );
        assert_eq!(
            env.get("ANTHROPIC_MODEL").map(String::as_str),
            Some("claude-sonnet-4-6")
        );
        assert_eq!(env.get("ANTHROPIC_DEFAULT_OPUS_MODEL_NAME"), None);
        assert_eq!(env.get("EMPTY_VALUE"), None);
    }
