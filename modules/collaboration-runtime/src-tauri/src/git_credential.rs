...
/// falls back to an Archipelago-owned platform data path if the flag is missing.
///
/// Paths are absolutized and shell-quoted before being substituted into the
/// script body so a quirky data dir (`ARCHIPELAGO_DATA_DIR=$HOME/x`, paths with
...
pub fn create_credential_helper_script(
    ...
    #[cfg(unix)]
    {
        let script_path = app_data_dir.join("git-credential-archipelago.sh");
        let content = format!(
            r#"#!/bin/sh
# Archipelago credential helper — calls the app binary to look up credentials.
...
"#,
    ...
    #[cfg(windows)]
    {
        let script_path = app_data_dir.join("git-credential-archipelago.bat");
        let content = format!(
            r#"@echo off
...
pub fn run_credential_helper() {
    ...
    // Archipelago helper scripts embed this so server deployments and custom
    // ARCHIPELAGO_DATA_DIR setups land on the right database.
    let explicit_data_dir = parse_data_dir_arg(std::env::args());

    // Pin ARCHIPELAGO_DATA_DIR for downstream lookups (notably the file-based
        ...
        .build()
    {
        Ok(rt) => rt,
        Err(e) => {
            eprintln!("[archipelago credential-helper] tokio runtime build failed: {e}");
            return;
        }
    };
    ...
    match rt.block_on(lookup_credential(&app_data_dir, &host)) {
        ...
        Err(e) => eprintln!("[archipelago credential-helper] lookup failed: {e}"),
    }
}
...
/// Look up a (username, token) pair for the given host using the Archipelago
...
fn resolve_app_data_dir() -> Option<std::path::PathBuf> {
    // On macOS: ~/Library/Application Support/app.archipelago.web
    // On Linux: ~/.local/share/app.archipelago.web
    // On Windows: %APPDATA%/app.archipelago.web
    #[cfg(target_os = "macos")]
    {
        dirs::data_dir().map(|d| d.join("app.archipelago.web"))
    }
    #[cfg(target_os = "linux")]
    {
        dirs::data_dir().map(|d| d.join("app.archipelago.web"))
...
case "$1" in
*[Uu]sername*) echo "$ARCHIPELAGO_GIT_USERNAME" ;;
*[Pp]assword*) echo "$ARCHIPELAGO_GIT_PASSWORD" ;;
esac
"#;
            std::fs::write(&script_path, content)?;
            // Make executable
...
if %errorlevel% equ 0 (
    echo %ARCHIPELAGO_GIT_USERNAME%
    exit /b
)
echo %1 | findstr /i "password" >nul
if %errorlevel% equ 0 (
    echo %ARCHIPELAGO_GIT_PASSWORD%
    exit /b
)
"#;
            std::fs::write(&script_path, content)?;
...
pub fn inject_credentials(
    ...
    cmd.env("GIT_CONFIG_COUNT", "1")
        ...
        .env("ARCHIPELAGO_GIT_USERNAME", username)
        .env("ARCHIPELAGO_GIT_PASSWORD", token)
...
mod tests {
    ...
    fn test_parse_data_dir_arg_space_separated() {
        let args = vec![
            ...
            "--data-dir".to_string(),
            "/tmp/archipelago-data".to_string(),
        ];
        assert_eq!(
            parse_data_dir_arg(args),
            Some(std::path::PathBuf::from("/tmp/archipelago-data"))
        );
    }
    ...
    fn test_parse_data_dir_arg_equals_form() {
        let args = vec![
            "archipelago".to_string(),
            "--data-dir=/tmp/archipelago-data".to_string(),
            "--credential-helper".to_string(),
        ];
        assert_eq!(
            parse_data_dir_arg(args),
    ...
    fn test_parse_data_dir_arg_absent() {
        let args = vec!["archipelago".to_string(), "--credential-helper".to_string()];
    ...
    fn test_parse_data_dir_arg_missing_value_treated_as_absent() {
        ...
        let args = vec!["archipelago".to_string(), "--data-dir".to_string()];
        assert_eq!(parse_data_dir_arg(args), None);
    }
    ...
    fn test_helper_script_embeds_data_dir() {
        let tmp =
            std::env::temp_dir().join(format!("archipelago-cred-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&tmp).expect("create tmp dir");
        let binary = std::path::PathBuf::from("/usr/local/bin/archipelago-server");
        ...
        // deployments don't fall back to the hardcoded `app.archipelago` path.
        // Paths are sh-single-quoted so spaces / `$` / backticks survive.
        assert!(content.contains("/usr/local/bin/archipelago-server"));
    ...
    fn test_helper_script_absolutizes_relative_data_dir() {
        // Regression: a relative `ARCHIPELAGO_DATA_DIR=data` previously produced
        ...
        let _guard = STATE_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let unique = format!("archipelago-rel-test-{}", uuid::Uuid::new_v4());
        ...
        let binary = std::path::PathBuf::from("./archipelago-server");
        ...
        assert!(
            !content.contains("'./archipelago-server'"),
    ...
    fn test_bat_double_quote_escapes_percent() {
        ...
        assert_eq!(
            bat_double_quote(r"C:\Users\%USERNAME%\archipelago"),
            "\"C:\\Users\\%%USERNAME%%\\archipelago\""
        );
    }
    ...
    fn test_helper_script_escapes_apostrophe_in_path() {
        ...
        let tmp =
            std::env::temp_dir().join(format!("archipelago-apos-test-{}", uuid::Uuid::new_v4()));
    ...
    fn test_credential_helper_e2e_server_mode() {
        ...
        let data_dir =
            std::env::temp_dir().join(format!("archipelago-helper-e2e-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&data_dir).expect("create data dir");

        // Save & override ARCHIPELAGO_DATA_DIR for the duration of this test —
        // server-mode `keyring_store` resolves `tokens.json` from this var.
        let saved_env = std::env::var("ARCHIPELAGO_DATA_DIR").ok();
        std::env::set_var("ARCHIPELAGO_DATA_DIR", &data_dir);
        ...
        rt.block_on(async {
            ...
            // Seed the file token store. Uses ARCHIPELAGO_DATA_DIR via
        ...
        match saved_env {
            Some(v) => std::env::set_var("ARCHIPELAGO_DATA_DIR", v),
            None => std::env::remove_var("ARCHIPELAGO_DATA_DIR"),
        }
