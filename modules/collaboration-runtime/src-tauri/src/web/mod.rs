...
impl WebServerState {
    ...
    /// callers (e.g. `archipelago-server`) can pass it to `build_router`.
    pub fn shutdown_signal(&self) -> Arc<ShutdownSignal> {
        self.shutdown_signal.clone()
    }

    /// Mark the server as running from outside the Tauri command path.
    /// `archipelago-server` calls `axum::serve` directly without going through
    ...
    /// True when the serve task is owned externally (e.g. by `archipelago-server`
...
pub fn find_static_dir_standalone(explicit: Option<&str>) -> PathBuf {
    if let Some(dir) = explicit {
        ...
        if p.join("index.html").exists() {
            eprintln!(
                "[WEB] Serving static files from ARCHIPELAGO_STATIC_DIR: {}",
                p.display()
            );
            return p;
        }
...
pub(crate) async fn do_start_web_server_tauri(
    ...
    let app_state = Arc::new(AppState {
        ...
        // `ARCHIPELAGO_DATA_DIR` reaches the credential helper and any HTTP
