...
pub const WS_EVENT_PROTOCOL: &str = "archipelago-events";
...
mod tests {
    ...
    fn parses_token_from_ws_protocols() {
        ...
        assert_eq!(
            token_from_ws_protocols(&format!("archipelago-events, archipelago-token.{encoded}"))
                .as_deref(),
            Some(token)
        );
    }
    ...
    fn ignores_invalid_ws_protocol_token() {
        assert!(
            token_from_ws_protocols("archipelago-events, archipelago-token.not-valid-@@@@")
                .is_none()
        );
    }
}
