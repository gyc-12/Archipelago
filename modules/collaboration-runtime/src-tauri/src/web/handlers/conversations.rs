...
use crate::commands::groups::{AgentDeletedPayload, AGENT_DELETED_EVENT, AGENT_UPSERTED_EVENT};
use crate::models::*;
...
pub async fn create_conversation(
    ...
    let db = &state.db;
    let (conversation_id, agents) = conv_commands::create_conversation_with_group_agent_core(
        &db.conn,
        params.folder_id,
        params.agent_type,
        params.title,
...
pub async fn delete_conversation(
    ...
    let agents = conv_commands::delete_conversation_with_group_agents_core(
        &state.db.conn,
        params.conversation_id,
    )
    .await?;
