...
use crate::paths::archipelago_uploads_root;
...
/// by the next.
const UPLOAD_TOTAL_BYTES_ENV: &str = "ARCHIPELAGO_UPLOAD_MAX_TOTAL_BYTES";

/// Opt-in fail-closed mode for the quota config. When truthy and
/// `ARCHIPELAGO_UPLOAD_MAX_TOTAL_BYTES` parses as `Invalid`, startup aborts
...
const UPLOAD_STRICT_ENV: &str = "ARCHIPELAGO_UPLOAD_QUOTA_STRICT";

/// Outcome of parsing `ARCHIPELAGO_UPLOAD_MAX_TOTAL_BYTES`. Carries enough
...
/// Strict-mode validation: returns `Err` when `ARCHIPELAGO_UPLOAD_QUOTA_STRICT`
...
///   * `archipelago-server` (single-purpose process) — call early in `main`
...
///
/// **Scope:** this counter is process-local. Multiple `archipelago-server`
...
/// own counter and can collectively exceed the cap. archipelago is designed
...
/// `paths::archipelago_uploads_root` for the operator-facing version of
/// this contract.
...
pub async fn purge_upload_staging() {
    let tmp_dir = archipelago_uploads_root().join(".tmp");
...
pub async fn upload_attachment(
    ...
    let uploads_root = archipelago_uploads_root();
...
mod tests {
    ...
    // `ARCHIPELAGO_UPLOAD_MAX_TOTAL_BYTES` from a test would race the harness's
    // parallel runner.
    ...
    fn upload_quota_strict_error_display_includes_env_names_and_value() {
        ...
        assert!(msg.contains("ARCHIPELAGO_UPLOAD_MAX_TOTAL_BYTES"));
        assert!(msg.contains("ARCHIPELAGO_UPLOAD_QUOTA_STRICT"));
