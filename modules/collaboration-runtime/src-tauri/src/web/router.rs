...
pub fn build_router(
    ...
    let api = Router::new()
        ...
        // ─── Island group chats ───
        .route("/list_groups", post(handlers::groups::list_groups))
        .route("/create_group", post(handlers::groups::create_group))
        .route("/update_group", post(handlers::groups::update_group))
        .route("/delete_group", post(handlers::groups::delete_group))
        .route("/add_group_agent", post(handlers::groups::add_group_agent))
        .route(
            "/remove_group_agent",
            post(handlers::groups::remove_group_agent),
        )
        .route(
            "/update_group_agent",
            post(handlers::groups::update_group_agent),
        )
        // ─── Window navigation ───
        .route(
        ...
        //
        // Issue #179: when archipelago runs in server mode the user has no
        ...
        .route(
            ...
            post(handlers::system_settings::reveal_path_in_file_manager),
        )
        .route(
