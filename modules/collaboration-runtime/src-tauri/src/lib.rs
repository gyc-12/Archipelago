...
mod tauri_app {
    ...
    use crate::commands::{
        ...
        folders, groups as group_commands, mcp as mcp_commands,
    ...
    fn notify_web_auto_start_failed(
        ...
        tauri::async_runtime::spawn(async move {
            let _ =
                notification::send_notification(app, "Archipelago Web service".to_string(), body)
                    .await;
        });
    }
    ...
    /// `~/.archipelago/preferences.json` so it can be read **before** the Tauri
    ...
    pub fn run() {
        ...
        process::ensure_common_cli_dirs_in_path();
        ...
        // can run alongside an installed release build of Archipelago during
        // development. Debug desktop builds use an isolated SQLite file, but
        // they still share other `app.archipelago.web` data-dir artifacts with release.
        ...
        builder
            ...
            // opened against a remote archipelago-server through Rust so we
            ...
            .setup(|app| {
                ...
                //   * `paths::archipelago_uploads_root` / `archipelago_pets_root`
                ...
                // to the same helper so a pre-set `ARCHIPELAGO_DATA_DIR` is
                // honored end-to-end.
                //
                ...
                //     `ARCHIPELAGO_DATA_DIR` would otherwise re-resolve
                ...
                // notification) do not read `ARCHIPELAGO_DATA_DIR`, and the
                ...
                // override.
                unsafe {
                    std::env::set_var("ARCHIPELAGO_DATA_DIR", &effective_data_dir);
                }

                // `ARCHIPELAGO_HOME` overrides `ARCHIPELAGO_DATA_DIR` inside
                // `paths::archipelago_uploads_root` / `archipelago_pets_root`.
                // If both are set and point at different roots,
                // uploads/pets land on `ARCHIPELAGO_HOME` while the database
                // lands on `ARCHIPELAGO_DATA_DIR` — a silent split. The
                ...
                // (`~/.archipelago/skills/`). Runs in the background and does
                ...
                // 60-second default via `ARCHIPELAGO_ACP_IDLE_TIMEOUT_SECS`
                // (set to `0` to disable).
                ...
                // Deep-link: register schemes at runtime on Windows/Linux
                // (macOS uses Info.plist, handled automatically by the plugin).
                #[cfg(any(windows, target_os = "linux"))]
                {
                    if let Err(err) = app.deep_link().register_all() {
                        eprintln!("[deep-link] failed to register schemes: {err}");
                    }
                }

                // Listen for incoming deep-link URLs and emit them to the
                // frontend webview so the workspace can navigate accordingly.
                {
                    use tauri::Emitter;
                    ...
                    app.deep_link().on_open_url(move |event| {
                        ...
                        for url in urls {
                            ...
                            let _ = handle.emit("deep-link://new-url", url_str);
                        }
                    });
                }
            ...
            .invoke_handler(tauri::generate_handler![
                ...
                group_commands::add_group_agent,
                group_commands::remove_group_agent,
                group_commands::update_group_agent,
