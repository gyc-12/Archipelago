...
mod tests {
    ...
    fn env_usize_uses_default_for_missing_invalid_and_zero() {
        temp_env::with_var_unset("ARCHIPELAGO_WORKSPACE_UPLOAD_MAX_CONCURRENCY", || {
            assert_eq!(
                env_usize("ARCHIPELAGO_WORKSPACE_UPLOAD_MAX_CONCURRENCY", 4),
                4
            );
        });
        temp_env::with_var(
