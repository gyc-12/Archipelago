...
fn path_contains(dir: &std::path::Path) -> bool {
    ...
    current
        .to_string_lossy()
        .split(sep)
        .any(|path| path == dir_str.as_ref())
}

fn prepend_to_path_if_needed(dir: &std::path::Path) {
    if dir.is_dir() && !path_contains(dir) {
        prepend_to_path(dir);
    }
}

fn append_to_path(dir: &std::path::Path) {
    let sep = if cfg!(windows) { ";" } else { ":" };
    let current = std::env::var_os("PATH").unwrap_or_default();
...
fn append_to_path_if_needed(dir: &std::path::Path) {
    if dir.is_dir() && !path_contains(dir) {
        append_to_path(dir);
    }
}

/// GUI-launched apps on macOS/Linux often inherit only a minimal system PATH.
/// Add common user CLI locations so child processes can find tools like
/// `claude`, `git`, and package-manager shims the same way an interactive shell
/// usually can.
pub fn ensure_common_cli_dirs_in_path() {
    if cfg!(windows) {
        return;
    }
    ...
    #[cfg(target_os = "macos")]
    {
        for dir in ["/opt/homebrew/bin", "/opt/homebrew/sbin", "/usr/local/bin"] {
            append_to_path_if_needed(std::path::Path::new(dir));
        }
    }
    ...
    if let Some(home) = dirs::home_dir() {
        for dir in [
            ...
            append_to_path_if_needed(&dir);
        }
    }
}

/// Return the user-local npm prefix directory (`~/.archipelago/npm-global/`).
///
...
pub(crate) fn user_npm_prefix() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".archipelago").join("npm-global"))
}
...
pub fn ensure_user_npm_prefix_in_path() {
    if let Some(prefix) = user_npm_prefix() {
        ...
        prepend_to_path_if_needed(&bin_dir);
    }
}
