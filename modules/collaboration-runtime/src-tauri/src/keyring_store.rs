...
const SERVICE_NAME: &str = "archipelago";
...
fn tokens_file_path() -> std::path::PathBuf {
    tokens_file_path_for(std::env::var("ARCHIPELAGO_DATA_DIR").ok().as_deref())
}

/// Resolve the on-disk `tokens.json` path given an explicit
/// `ARCHIPELAGO_DATA_DIR` value (or `None` to fall back to the platform
...
fn tokens_file_path_for(env_value: Option<&str>) -> std::path::PathBuf {
    let dir = env_value.map(std::path::PathBuf::from).unwrap_or_else(|| {
        dirs::data_dir()
            ...
            .unwrap_or_else(|| std::path::PathBuf::from(".archipelago-data"))
    });
...
mod tests {
    ...
    fn test_tokens_file_path_absolutizes_relative_env() {
        // Regression: a relative `ARCHIPELAGO_DATA_DIR=data` previously made
