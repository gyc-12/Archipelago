...
//   2. Runs `cargo build --release --bin archipelago-mcp --no-default-features`
//      for that triple from `src-tauri/`.
//   3. Copies the produced binary to
//      `src-tauri/binaries/archipelago-mcp-<triple>{.exe}` so Tauri's externalBin
//      bundler picks it up under the bare name `archipelago-mcp` at install time.
//
...
//   - Skippable: set `ARCHIPELAGO_SKIP_SIDECAR=1` when iterating on the frontend
...
const BIN_NAME = "archipelago-mcp"
...
function main() {
  if (process.env.ARCHIPELAGO_SKIP_SIDECAR === "1") {
    log("ARCHIPELAGO_SKIP_SIDECAR=1 — skipping sidecar preparation")
    return
  }
  ...
  // `--no-default-features` keeps archipelago-mcp free of the Tauri runtime deps —
