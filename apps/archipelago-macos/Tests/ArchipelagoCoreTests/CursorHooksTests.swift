...
struct CursorHooksTests {
    ...
    func cursorHookInstallerInstallsIntoEmptyFile() throws {
        let mutation = try CursorHookInstaller.installHooksJSON(
            existingData: nil,
            hookCommand: "/usr/local/bin/ArchipelagoHooks --source cursor"
        )
        ...
        #expect(shellEntries[0]["command"] as? String == "/usr/local/bin/ArchipelagoHooks --source cursor")
    }
    ...
    func cursorHookInstallerPreservesExistingHooks() throws {
        ...
        #expect(shellEntries[1]["command"] as? String == "/usr/local/bin/ArchipelagoHooks --source cursor")
    }
    ...
    func cursorHookInstallerUninstallsCleanly() throws {
        ...
        let uninstalled = try CursorHookInstaller.uninstallHooksJSON(
            ...
            managedCommand: "/usr/local/bin/ArchipelagoHooks --source cursor"
        )
