...
struct HookSkipConfigurationTests {
    /// Accepts common truthy spellings for the preferred Archipelago key.
    /// 推荐的新环境变量接受常见真值写法。
    @Test
    func archipelagoSkipHooksAcceptsTruthyValues() {
        for value in ["1", "true", "TRUE", "yes", "on", " 1 "] {
            #expect(HookSkipConfiguration.shouldSkipHooks(environment: [
                HookSkipConfiguration.archipelagoSkipKey: value,
            ]))
        }
    }

    /// Keeps compatibility with existing Open Island-based wrappers.
    /// 保留对已有 Open Island wrapper 的兼容。
    @Test
    func legacyOpenIslandSkipAliasIsSupported() {
        #expect(HookSkipConfiguration.shouldSkipHooks(environment: [
            HookSkipConfiguration.legacyOpenIslandSkipKey: "1",
        ]))
    }
