...
ICONSET_DIR = BRAND_ROOT / "Archipelago.iconset"
...
ICNS_PATH = BRAND_ROOT / "Archipelago.icns"
...
def write_app_icons() -> None:
    # v7 Island+Agents master is the source for the current app icon.
    source_path = BRAND_ROOT / "app-icon-v7.png"
    if not source_path.exists():
        raise SystemExit(
            f"missing brand master at {source_path.relative_to(REPO_ROOT)} — "
            "choose and save the current app icon master first"
        )
