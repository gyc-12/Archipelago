...
final class HookInstallationCoordinator {
    ...
    var claudeHookStatusSummary: String {
        ...
        if hooksBinaryURL == nil {
            return "Build ArchipelagoHooks before installing."
        }
    ...
    var claudeUsageStatusSummary: String {
        ...
        if status.managedStatusLineNeedsRepair {
            return "Archipelago detected a missing managed Claude status line script and will repair it automatically."
        }

        if status.hasConflictingStatusLine {
            return "Archipelago will not overwrite an existing Claude status line automatically."
        }
    ...
    func installCodexHooks() {
        guard let hooksBinaryURL else {
            onStatusMessage?("Could not find a local ArchipelagoHooks binary. Build the package first.")
            return
        }
    ...
    private func loadBundledOpenCodePlugin() -> Data? {
        ...
        if let url = Bundle.appResources.url(forResource: "archipelago-opencode", withExtension: "js") {
            return try? Data(contentsOf: url)
        }

        // Fallback: Bundle.main for Xcode builds
        if let url = Bundle.main.url(forResource: "archipelago-opencode", withExtension: "js") {
            return try? Data(contentsOf: url)
        }
