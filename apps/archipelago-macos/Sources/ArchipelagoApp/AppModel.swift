...
final class AppModel {
    ...
    private struct ArchipelagoClosedIslandSnapshot: Equatable {
        var groupCount: Int = 0
        var hasWorkingAgent: Bool = false
    }
    ...
    private var archipelagoClosedIslandSnapshot = ArchipelagoClosedIslandSnapshot()
    ...
    let archipelago = ArchipelagoCoordinator()
    let chatWindows = ChatWindowController()
    ...
    var groupChatSort: GroupChatSort {
        get { appearancePreferences(for: activeAppearanceProfile).groupChatSort }
        set { updateAppearancePreferences(for: activeAppearanceProfile) { $0.groupChatSort = newValue } }
    }

    var listSpacing: GroupChatListSpacing {
        get { appearancePreferences(for: activeAppearanceProfile).listSpacing }
        set { updateAppearancePreferences(for: activeAppearanceProfile) { $0.listSpacing = newValue } }
    }

    var agentBadgeDisplay: AgentBadgeDisplay {
        get { appearancePreferences(for: activeAppearanceProfile).agentBadgeDisplay }
        set { updateAppearancePreferences(for: activeAppearanceProfile) { $0.agentBadgeDisplay = newValue } }
    }

    // MARK: - Legacy session list preferences (hardcoded, not user-configurable)

    var islandUsageDisplay: IslandUsageDisplay { .compact }
    ...
    // Hardcoded default for stale threshold (removed from user preferences)
    ...
    private func appearancePreferencesDidChange(
        ...
        if oldValue.groupChatSort != newValue.groupChatSort {
            _cachedSessionBuckets = nil
        }
        refreshOverlayPlacementIfVisible()
    }
    ...
    private func persistAppearancePreferences(
        ...
        defaults.set(preferences.groupChatSort.rawValue, forKey: Self.appearanceDefaultsKey(profile, "groupChatSort"))
        defaults.set(preferences.listSpacing.rawValue, forKey: Self.appearanceDefaultsKey(profile, "listSpacing"))
        defaults.set(preferences.agentBadgeDisplay.rawValue, forKey: Self.appearanceDefaultsKey(profile, "agentBadgeDisplay"))
    }
    ...
    private static func loadAppearancePreferences(for profile: IslandAppearanceDisplayProfile) -> IslandAppearancePreferences {
        ...
        return IslandAppearancePreferences(
            ...
            groupChatSort: GroupChatSort(
                rawValue: defaults.string(forKey: appearanceDefaultsKey(profile, "groupChatSort"))
                    ?? defaults.string(forKey: legacyIslandSessionSortDefaultsKey)
                    ?? ""
            ) ?? .recentActivity,
            listSpacing: GroupChatListSpacing(
                rawValue: defaults.string(forKey: appearanceDefaultsKey(profile, "listSpacing")) ?? ""
            ) ?? .standard,
            agentBadgeDisplay: AgentBadgeDisplay(
                rawValue: defaults.string(forKey: appearanceDefaultsKey(profile, "agentBadgeDisplay")) ?? ""
            ) ?? .all
        )
    }
    ...
    init(
        ...
        syncArchipelagoClosedIslandSnapshot()
        archipelago.onGroupChatsChanged = { [weak self] in
            self?.syncArchipelagoClosedIslandSnapshot()
        }
        archipelago.onAgentTurnCompleted = { [weak self] completion in
            self?.presentArchipelagoTurnCompletion(completion)
        }
    ...
    /// Measured by SwiftUI for the current Archipelago group-chat page. This lets
    /// the Island grow to fit detail and agent-management pages instead of
    /// relying on stale fixed estimates.
    var measuredArchipelagoContentHeight: CGFloat = 0 {
        didSet {
            let delta = abs(measuredArchipelagoContentHeight - oldValue)
            if delta >= 2 {
                overlay.refreshOverlayPlacementIfVisible()
            }
        }
    ...
    var islandClosedMode: UnifiedBars.Mode {
        ...
        if sessions.contains(where: { $0.phase == .running })
            || hasWorkingArchipelagoAgentForClosedIsland {
            return .running
        }
        return .idle
    }
    ...
    func islandClosedRightSlotContent() -> IslandRightSlotContent? {
        let archipelagoCount = archipelagoGroupChatCountForClosedIsland
        let sessions = surfacedSessions
        ...
        case .agents:
            // 根据群聊数量和状态生成方块网格
            let groups = archipelago.groupChats
            guard !groups.isEmpty else {
                ...
                if n <= 9 {
                    ...
                    cells.append(.overflow(n - 7))
                }
                return .agents(cells)
            }

            // 为每个群聊生成方块，状态根据群聊中的 agent 状态决定
            var cells: [AgentGridCell] = []
            let displayGroups = groups.count <= 9 ? groups : Array(groups.prefix(7))

            for group in displayGroups {
                // 检查群聊中是否有 agent 在工作
                let hasWorking = group.agents.contains { agent in
                    agent.status == .prompting
                }
                let state: AgentGridCellState = hasWorking ? .running : .idle
                cells.append(.session(color: .yellow, state: state))
            }

            if groups.count > 9 {
                cells.append(.overflow(groups.count - 7))
            }
    ...
    private var archipelagoGroupChatCountForClosedIsland: Int {
        archipelagoClosedIslandSnapshot.groupCount
    }

    private var hasWorkingArchipelagoAgentForClosedIsland: Bool {
        archipelagoClosedIslandSnapshot.hasWorkingAgent
    }

    private func syncArchipelagoClosedIslandSnapshot() {
        let snapshot = ArchipelagoClosedIslandSnapshot(
            groupCount: archipelago.groupChats.count,
            hasWorkingAgent: archipelago.groupChats.contains { group in
                group.agents.contains { $0.displayStatus == .working }
            }
        )
        guard archipelagoClosedIslandSnapshot != snapshot else { return }
        archipelagoClosedIslandSnapshot = snapshot
        refreshOverlayPlacementIfVisible()
    }
    ...
    private func presentArchipelagoTurnCompletion(_ completion: ArchipelagoAgentTurnCompletion) {
        archipelago.navigateToDetail(groupId: completion.groupId)
        lastActionMessage = "\(completion.groupName) · \(completion.agentType.displayName) 已完成回复。"
        NotificationSoundService.playNotification(isMuted: isSoundMuted)
        notchOpen(reason: .notification, surface: .sessionList())
    }
    ...
    func showSettings() {
        ...
        if let window = NSApp.windows.first(where: { $0.title == "Archipelago Settings" || $0.title == "Open Island Settings" }) {
    ...
    func approvePermission(for sessionID: String, action: ApprovalAction) {
        ...
        switch action {
        case .deny:
            resolution = .deny(message: "Permission denied in Archipelago.", interrupt: false)
    ...
    private func permissionResolution(for approved: Bool) -> PermissionResolution {
        ...
        return .deny(message: "Permission denied in Archipelago.", interrupt: false)
    }
