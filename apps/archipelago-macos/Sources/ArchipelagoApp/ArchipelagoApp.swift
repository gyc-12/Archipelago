...
@MainActor
final class ArchipelagoAppDelegate: NSObject, NSApplicationDelegate {
    let model = AppModel()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ProcessInfo.processInfo.disableAutomaticTermination(
            "Archipelago should remain active while monitoring local agent sessions."
        )
        ...
        DispatchQueue.main.async { [self] in
            ...
            ArchipelagoAppDelegate.hideAllAppWindows()
...
struct ArchipelagoApp: App {
    @NSApplicationDelegateAdaptor(ArchipelagoAppDelegate.self)
    private var appDelegate

    @Environment(\.openWindow) private var openWindow

    var body: some Scene {
        Window("Archipelago Settings", id: "settings") {
