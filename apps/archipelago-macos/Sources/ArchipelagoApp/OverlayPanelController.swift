...
final class OverlayPanelController {
    ...
    private static let archipelagoChatListMinHeight: CGFloat = 220
    private static let archipelagoCreateChatHeight: CGFloat = 360
    private static let archipelagoAddAgentsHeight: CGFloat = 320
    private static let archipelagoGroupDetailBaseHeight: CGFloat = 160
    private static let archipelagoGroupDetailAgentRowHeight: CGFloat = 52
    private static let archipelagoGroupDetailSummaryRowHeight: CGFloat = 96
    ...
    private func panelSize(for model: AppModel?, on screen: NSScreen) -> CGSize {
        ...
        let minPanelHeight = screen.notchSize.height + Self.openedEmptyStateHeight + Self.openedContentBottomPadding + insets.bottom
        let maxPanelHeight = max(minPanelHeight, screen.visibleFrame.height - 12)
        let height = min(rawHeight, maxPanelHeight)
    ...
    private func openedContentHeight(for model: AppModel) -> CGFloat {
        if model.measuredArchipelagoContentHeight > 0 {
            return model.measuredArchipelagoContentHeight
        }

        if let archipelagoHeight = Self.estimatedArchipelagoContentHeight(
            contentState: model.archipelago.contentState,
            groupChats: model.archipelago.groupChats
        ) {
            return archipelagoHeight
        }
    ...
    static func estimatedArchipelagoContentHeight(
        contentState: ArchipelagoContentState,
        groupChats: [GroupChat]
    ) -> CGFloat? {
        switch contentState {
        case .chatList:
            let visibleGroups = Array(groupChats.prefix(4))
            let rowHeights = visibleGroups.map { group in
                group.latestCompletedAgent?.latestResponseSummary?.nilIfBlank == nil ? CGFloat(62) : CGFloat(106)
            }
            let rowsHeight = rowHeights.isEmpty ? CGFloat(62) : rowHeights.reduce(0, +)
            let spacingHeight = CGFloat(max(rowHeights.count - 1, 0)) * 6
            return max(Self.archipelagoChatListMinHeight, 56 + rowsHeight + spacingHeight)
        case .createChat:
            return Self.archipelagoCreateChatHeight
        case .addAgents:
            return Self.archipelagoAddAgentsHeight
        case .groupDetail(let groupId):
            let agents = groupChats.first(where: { $0.id == groupId })?.agents ?? []
            let rowHeights = agents.map { agent in
                agent.latestResponseSummary?.nilIfBlank == nil
                    ? Self.archipelagoGroupDetailAgentRowHeight
                    : Self.archipelagoGroupDetailSummaryRowHeight
            }
            let rowsHeight = rowHeights.isEmpty
                ? Self.archipelagoGroupDetailAgentRowHeight
                : rowHeights.reduce(0, +)
            let spacingHeight = CGFloat(max(rowHeights.count - 1, 0)) * 6
            let contentHeight = Self.archipelagoGroupDetailBaseHeight + rowsHeight + spacingHeight
            return max(contentHeight, Self.archipelagoChatListMinHeight)
        }
    }
