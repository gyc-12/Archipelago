// Archipelago plugin for OpenCode
// Bridges OpenCode events to the Archipelago desktop app via Unix socket.
// Install: copy to ~/.config/opencode/plugins/archipelago.js
import { connect } from "net";
import { appendFileSync } from "fs";
import { homedir } from "os";

const DEBUG_LOG = "/tmp/archipelago-opencode-debug.log";
function debugLog(msg) {
  try { appendFileSync(DEBUG_LOG, `[${new Date().toISOString()}] ${msg}\n`); } catch {}
}

const SOCKET_PATH =
  process.env.ARCHIPELAGO_SOCKET_PATH ||
  process.env.OPEN_ISLAND_SOCKET_PATH ||
  `${process.env.HOME || homedir()}/Library/Application Support/Archipelago/bridge.sock`;
  ...
  return {
    "event": async ({ event }) => {
      try {
        ...
        // Fail open: if Archipelago is unavailable, don't block OpenCode
      }
    },

    "shell.env": async (input, output) => {
      output.env.ARCHIPELAGO_ACTIVE = "1";
      for (const v of ENV_KEYS) {
        if (process.env[v]) output.env["_ARCHIPELAGO_" + v] = process.env[v];
      }
    },
  };
};
