...
struct IslandPanelView: View {
    ...
    private func openedSurface(width openedWidth: CGFloat, height openedHeight: CGFloat) -> some View {
        ...
        ZStack(alignment: .top) {
            ...
            VStack(spacing: 0) {
                ...
                openedContent(maxHeight: max(0, openedHeight - closedNotchHeight))
    ...
    private func openedContent(maxHeight: CGFloat) -> some View {
        AutoHeightScrollView(
            maxHeight: maxHeight,
            onContentHeightChange: { height in
                model.measuredArchipelagoContentHeight = height
            }
        ) {
            archipelagoOpenedContent
        }
        .animation(.spring(response: 0.35, dampingFraction: 0.85), value: model.archipelago.contentState)
        .padding(.bottom, 0)
        .onChange(of: model.archipelago.contentState) { _, _ in
            model.measuredArchipelagoContentHeight = 0
        }
    }

    @ViewBuilder
    private var archipelagoOpenedContent: some View {
        VStack(spacing: 0) {
            switch model.archipelago.contentState {
            case .chatList:
                GroupChatListView(
                    coordinator: model.archipelago,
                    sortOrder: model.groupChatSort,
                    spacing: model.listSpacing,
                    badgeDisplay: model.agentBadgeDisplay,
                    onOpenDetail: { group in
                        model.archipelago.navigateToDetail(groupId: group.id)
                    },
                    onOpenChat: { group in
                        model.chatWindows.openGroupChat(group: group, coordinator: model.archipelago)
                    }
                )
                .transition(.asymmetric(insertion: .move(edge: .leading), removal: .move(edge: .trailing)))
            case .createChat:
                CreateGroupChatView(coordinator: model.archipelago)
                    .transition(.asymmetric(insertion: .move(edge: .trailing), removal: .move(edge: .leading)))
            case .addAgents(let groupId):
                AddAgentsView(groupId: groupId, coordinator: model.archipelago)
                    .transition(.asymmetric(insertion: .move(edge: .trailing), removal: .move(edge: .leading)))
            case .groupDetail(let groupId):
                GroupDetailView(groupId: groupId, coordinator: model.archipelago) {
                    if let group = model.archipelago.group(byId: groupId) {
                        model.chatWindows.openGroupChat(group: group, coordinator: model.archipelago)
                    }
                } onOpenAgentChat: { group, agent in
                    model.chatWindows.openGroupChat(group: group, agent: agent, coordinator: model.archipelago)
                }
                .transition(.asymmetric(insertion: .move(edge: .trailing), removal: .move(edge: .leading)))
            }
        }
