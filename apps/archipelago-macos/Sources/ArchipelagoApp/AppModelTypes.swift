...
struct IslandAppearancePreferences: Equatable, Sendable {
    ...
    var groupChatSort: GroupChatSort = .recentActivity
    var listSpacing: GroupChatListSpacing = .standard
    var agentBadgeDisplay: AgentBadgeDisplay = .all
}

enum GroupChatSort: String, CaseIterable, Identifiable, Sendable {
    case name            // 按名称 A-Z
    case recentActivity  // 按最近活动
...
// MARK: - Legacy session list preferences (not exposed in UI, hardcoded defaults)
