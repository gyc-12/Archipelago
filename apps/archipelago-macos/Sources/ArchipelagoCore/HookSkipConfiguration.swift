...
public enum HookSkipConfiguration {
    /// Preferred Archipelago environment key for disabling hooks in this process.
    /// Archipelago 推荐使用的当前进程 hook 跳过开关。
    public static let archipelagoSkipKey = "ARCHIPELAGO_SKIP_HOOKS"
    /// Compatibility alias used by existing Open Island integrations.
    /// 兼容已有 Open Island 集成使用的旧开关。
    ...
    public static func shouldSkipHooks(environment: [String: String]) -> Bool {
        isTruthy(environment[archipelagoSkipKey])
